...
//! UI-facing NGauge STOQ API handlers.
//!
//! Handlers for endpoints expected by the UI TypeScript interfaces in
//! `ui/frontend/lib/api/services/NGaugeAPI.ts` that go beyond the core
...
use super::stoq_api::{
    ApiError, ApiHandler, ApiRequest, ApiResponse, CreateLeaseRequest, NGaugeAppState,
...
impl ApiHandler for TrendingHandler {
    async fn handle(&self, request: ApiRequest) -> Result<ApiResponse, ApiError> {
        debug!("Handling ngauge/trending: {}", request.id);
...
impl ApiHandler for ThrottleHandler {
    async fn handle(&self, request: ApiRequest) -> Result<ApiResponse, ApiError> {
        debug!("Handling ngauge/throttle: {}", request.id);
...
impl ApiHandler for RoutingAdvisoryHandler {
    async fn handle(&self, request: ApiRequest) -> Result<ApiResponse, ApiError> {
        debug!("Handling ngauge/routing_advisory: {}", request.id);
...
impl ApiHandler for MarketplacePoolsHandler {
    async fn handle(&self, request: ApiRequest) -> Result<ApiResponse, ApiError> {
        debug!("Handling ngauge/marketplace_pools: {}", request.id);
...
impl ApiHandler for MarketplaceLeasesHandler {
    async fn handle(&self, request: ApiRequest) -> Result<ApiResponse, ApiError> {
        debug!("Handling ngauge/marketplace_leases: {}", request.id);
...
impl ApiHandler for MarketplacePricingHandler {
    async fn handle(&self, request: ApiRequest) -> Result<ApiResponse, ApiError> {
        debug!("Handling ngauge/marketplace_pricing: {}", request.id);
...
impl ApiHandler for MetricsStreamHandler {
    async fn handle(&self, request: ApiRequest) -> Result<ApiResponse, ApiError> {
        debug!("Handling ngauge/metrics_stream: {}", request.id);
...
impl ApiHandler for CreateLeaseHandler {
    async fn handle(&self, request: ApiRequest) -> Result<ApiResponse, ApiError> {
        debug!("Handling ngauge/create_lease: {}", request.id);
