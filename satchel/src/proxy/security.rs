...
impl QuantumSecurity {
    ...
    fn generate_token_id(&self, proxy_addr: &ProxyAddress) -> String {
    ...
    async fn validate_token_payload(&self, payload: &[u8]) -> AssetResult<bool> {
        ...
        if payload.len() >= 16 {
            ...
            let timestamp = u64::from_le_bytes(timestamp_bytes.try_into().unwrap_or([0u8; 8]));
...
mod tests {
    ...
    async fn test_quantum_security_creation() {
        let security = QuantumSecurity::new().await.unwrap();
        assert_eq!(security.active_tokens.len(), 0);
    }
    
    #[tokio::test]
    async fn test_falcon_signer() {
        let signer = FalconSigner::new().unwrap();
        let test_data = b"test message for signing";
        
        let signature = signer.sign(test_data).await.unwrap();
        assert!(!signature.is_empty());
        
        let valid = signer.verify(test_data, &signature).await.unwrap();
        assert!(valid);
        
        // Test with different data - should not verify
        let invalid = signer.verify(b"different message", &signature).await.unwrap();
        assert!(!invalid);
    }
    
    #[tokio::test]
    async fn test_kyber_encryption() {
        let kyber = KyberEncryption::new().unwrap();
        let test_data = b"sensitive data for encryption";
        
        let encrypted = kyber.encrypt(test_data).await.unwrap();
        assert_ne!(encrypted, test_data);
        
        let decrypted = kyber.decrypt(&encrypted).await.unwrap();
    ...
    async fn test_access_token_generation_and_validation() {
        ...
        let tokens = security.generate_access_tokens(&proxy_addr).await.unwrap();
        assert!(!tokens.is_empty());
        
        let valid = security.validate_access_tokens(&tokens).await.unwrap();
        assert!(valid);
    }
}