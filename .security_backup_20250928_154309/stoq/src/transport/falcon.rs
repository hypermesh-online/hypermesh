...
mod tests {
    ...
    fn test_falcon_sign_verify() {
        ...
        let verification = engine.verify(&public_key, &signature, message).unwrap();
        assert!(verification, "Signature verification should succeed");

        // Test with wrong message
        let wrong_message = b"Different message";
        let verification = engine.verify(&public_key, &signature, wrong_message).unwrap();
        assert!(!verification, "Signature verification should fail for wrong message");
    }

    #[test]
    fn test_falcon_transport_handshake() {
        ...
        let public_key = transport.get_local_public_key().unwrap().clone();
        transport.add_trusted_key("test_key".to_string(), public_key);

        let verification = transport.verify_handshake_signature("test_key", &signature, handshake_data).unwrap();
    ...
    fn test_signature_wire_format() {
        ...
        let (private_key, _) = engine.generate_keypair().unwrap();
        ...
        let imported = transport.import_signature(&exported).unwrap();
