...
//! NGauge SSE streaming endpoint (Phase M.6b).
//!
//! This module hosts the gateway-side Server-Sent Events stream for ngauge
//! telemetry. Clients connect via `GET /api/v1/blockmatrix/ngauge/stream`
//! (or the alias `/api/v1/ngauge/stream`) carrying an `X-HyperMesh-Capability`
//! header (validated only when capability tokens land; in alpha we accept
//! anonymous reads).
//!
//! ## Design rationale
//!
//! The ngauge daemon currently speaks a custom QUIC stream protocol (JSON
//! request/response over `quinn` bidi streams) and does **not** speak HTTP/3.
//! Forwarding HTTP/SSE through the existing `Http3Proxy` would require ngauge
//! to terminate HTTP, which it does not.
//!
//! Until ngauge gains a full HTTP/3 listener, the gateway hosts the SSE
//! endpoint locally. It owns an [`ngauge::streaming::SseFrameBridge`] and
...
//! ngauge daemon over its native stream protocol.
//!
//! Streaming semantics:
...
pub const SSE_PATH_BLOCKMATRIX: &str = "/api/v1/blockmatrix/ngauge/stream";
/// Alias under `/api/v1/ngauge` (convenience for direct consumers).
pub const SSE_PATH_NGAUGE: &str = "/api/v1/ngauge/stream";
...
/// Returns true if `path` targets the ngauge SSE endpoint.
pub fn is_sse_path(path: &str) -> bool {
    path == SSE_PATH_BLOCKMATRIX || path == SSE_PATH_NGAUGE
}
...
/// Shared ngauge SSE streaming state held by the gateway router.
///
...
#[derive(Clone)]
pub struct NGaugeStream {
    bridge: SseFrameBridge,
    require_capability: Arc<std::sync::atomic::AtomicBool>,
}

impl NGaugeStream {
    /// Build a new ngauge SSE stream backed by an internal bridge.
    ///
    ...
    /// Returns the underlying bridge so producers (e.g. ngauge integration)
    ...
    pub fn spawn_stub_producer(&self) -> tokio::task::JoinHandle<()> {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                // real ngauge integration lands, replace this with frames
                // pulled from the ngauge daemon's publisher.
                ...
                if let Some(frame) = bridge.publish_capacity(&metrics) {
                    debug!("ngauge SSE: published stub frame seq={}", frame.sequence);
                }
    ...
    pub fn handle(&self, headers: &HeaderMap) -> SseHandshake {
        ...
        info!("ngauge SSE subscriber connected, total={}", self.bridge.subscriber_count());
        ...
        tokio::select! {
            recv = rx.recv() => match recv {
                Ok(frame) => {
                    if let Some(payload) = SseFrameBridge::encode_sse(&frame) {
                        if chunk_tx.send(Bytes::from(payload)).await.is_err() {
                            debug!("ngauge SSE: client disconnected, ending pump");
                            return;
                        }
                    }
                }
                Err(RecvError::Lagged(skipped)) => {
                    warn!("ngauge SSE subscriber lagged by {skipped} frames");
                ...
                Err(RecvError::Closed) => {
                    debug!("ngauge SSE: broadcast channel closed");
                    return;
                }
            },
            _ = keepalive.tick() => {
                if chunk_tx
                    ...
                    .is_err()
                {
                    debug!("ngauge SSE: client disconnected during keepalive");
                    return;
                }
            }
        }
...
mod tests {
    ...
    fn make_stream() -> NGaugeStream {
        NGaugeStream::new(NodeId::from_public_key(b"gw-test"), PrivacyMode::PUBLIC)
    }

    #[test]
    fn path_detection_matches_blockmatrix_and_alias() {
        assert!(is_sse_path("/api/v1/blockmatrix/ngauge/stream"));
        assert!(is_sse_path("/api/v1/ngauge/stream"));
        assert!(!is_sse_path("/api/v1/blockmatrix/ngauge/foo"));
    ...
    async fn stub_producer_emits_frames_on_interval() {
        ...
        let first = tokio::time::timeout(Duration::from_secs(6), rx.recv())
            ...
            .expect("test: recv ok");
        match first.payload {
            ngauge::streaming::MetricsPayload::Capacity(_) => {}
