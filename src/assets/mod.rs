...
impl HyperMeshAssetLayer {
    ...
    /// Dashboard API: List assets with optional filtering
    pub async fn list_assets(&self, filter: Option<String>, limit: Option<u32>) -> Result<Vec<serde_json::Value>> {
        ...
        for asset_entry in self.assets.iter() {
            ...
            let asset = asset_entry.value();
            
            // Apply filter if provided
            if let Some(ref filter_str) = filter {
                if !asset.name.contains(filter_str) && 
                   !asset.description.contains(filter_str) &&
                   !asset.asset_type.to_string().contains(filter_str) {
                    continue;
                }
            }
            ...
            let asset_info = serde_json::json!({
                ...
                "capacity": {
                    "total": asset.allocation.total_capacity,
                    "available": asset.allocation.available_capacity,
                    "allocated": asset.allocation.allocated_capacity,
                },
                "proxy_address": asset.proxy_address,
                "created_at": asset.created_at.duration_since(SystemTime::UNIX_EPOCH).unwrap().as_secs(),
                "updated_at": asset.updated_at.duration_since(SystemTime::UNIX_EPOCH).unwrap().as_secs(),
            });
    ...
    /// Dashboard API: Get detailed asset information
    pub async fn get_asset_details(&self, asset_id: &str) -> Result<serde_json::Value> {
        let asset = self.assets.get(asset_id)
            .ok_or_else(|| anyhow!("Asset not found: {}", asset_id))?;
        
        let mut active_allocations = Vec::new();
        for allocation_entry in self.allocations.iter() {
            let allocation = allocation_entry.value();
            if allocation.request.asset_id == asset_id {
                active_allocations.push(serde_json::json!({
                    "id": allocation.id,
                    "amount": allocation.allocated_amount,
                    "user": allocation.request.requester_id,
                    "created_at": allocation.allocated_at.duration_since(SystemTime::UNIX_EPOCH).unwrap().as_secs(),
                }));
            }
        }
        ...
        Ok(serde_json::json!({
            ...
            "consensus_proofs": asset.consensus_proofs,
    ...
    /// Dashboard API: Create new asset
    pub async fn create_asset(&self, asset_type: &str, specifications: serde_json::Value) -> Result<String> {
        let asset_type_enum = match asset_type {
            "cpu" => AssetType::Cpu,
            "gpu" => AssetType::Gpu,
            ...
            _ => return Err(anyhow!("Unknown asset type: {}", asset_type)),
        };
        
        // Convert JSON specifications to HashMap
        let mut spec_map = HashMap::new();
        if let serde_json::Value::Object(obj) = specifications {
        ...
        // Build Asset struct
        let asset = Asset {
            id: String::new(), // Will be generated by register_asset
            asset_type: asset_type_enum,
            name: format!("Dashboard Created {}", asset_type),
            description: format!("Asset created via dashboard API"),
        ...
        let asset_id = self.register_asset(asset).await?;
        
        Ok(asset_id)
    }
    
    /// Dashboard API: Update asset specifications
    pub async fn update_asset(&self, asset_id: &str, specifications: serde_json::Value) -> Result<()> {
        let asset = self.assets.get(asset_id)
            .ok_or_else(|| anyhow!("Asset not found: {}", asset_id))?;
        
        let mut updated_asset = (**asset).clone();
        if let serde_json::Value::Object(spec_map) = specifications {
            for (key, value) in spec_map {
                updated_asset.specifications.insert(key, value);
            }
        }
        updated_asset.updated_at = SystemTime::now();
        
        self.assets.insert(asset_id.to_string(), Arc::new(updated_asset));
        
        Ok(())
    }
    
    /// Dashboard API: Delete asset
    pub async fn delete_asset(&self, asset_id: &str) -> Result<()> {
        // Check for active allocations
        let mut has_active_allocations = false;
        for allocation_entry in self.allocations.iter() {
            if allocation_entry.value().request.asset_id == asset_id {
        ...
        if has_active_allocations {
            return Err(anyhow!("Cannot delete asset with active allocations: {}", asset_id));
        }
