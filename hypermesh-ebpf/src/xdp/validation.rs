...
/// kernel policy struct does not carry them) and are enforced in the
/// userspace fallback path, so they are not serialized here.
...
mod map_serializer_tests {
    ...
    // not require a running kernel — the kernel DROP == userspace-reject
