...
/// packets at wire speed (wrong algorithm byte, stale cache entries).
/// Full asymmetric crypto verification (FALCON-1024, Ed25519, ECDSA)
/// MUST remain in userspace because the BPF instruction set has no such
/// helpers.
///
/// There is deliberately **no difficulty setting**: PoWork is the hash of the
/// work done (a content-hash match), not a mining contest, so there is no
/// threshold for the kernel to enforce.
///
/// Serialization layout (24 bytes, little-endian, natural alignment — matches
/// C `struct pos_config`; u64 fields are 8-byte aligned):
///   `[0..8]`   max_timestamp_skew_ns (u64 LE)
///   `[8..16]`  validation_ttl_ns     (u64 LE)
///   `[16..20]` enabled               (u32 LE, 1 or 0)
///   `[20..24]` (padding)
...
impl KernelPosConfig {
    /// Serialized size matching the C `struct pos_config` natural layout.
    pub const SIZE: usize = 24;

    /// Serialize to 24 bytes matching the C `struct pos_config` NATURAL
    /// (non-packed) layout — the u64 fields are 8-byte aligned, so the struct
    /// carries 4 trailing pad bytes.
    ///
    /// Layout (all little-endian; matches `struct pos_config` in hypermesh_xdp.c):
    ///   `[0..8]`   max_timestamp_skew_ns u64
    ///   `[8..16]`  validation_ttl_ns     u64
    ///   `[16..20]` enabled               u32
    ///   `[20..24]` (padding)
    pub fn to_bytes(&self) -> [u8; Self::SIZE] {
        let mut buf = [0u8; Self::SIZE];
        buf[0..8].copy_from_slice(&self.max_timestamp_skew_ns.to_le_bytes());
        buf[8..16].copy_from_slice(&self.validation_ttl_ns.to_le_bytes());
        buf[16..20].copy_from_slice(&(self.enabled as u32).to_le_bytes());
        buf
    }

    /// Deserialize from 24 bytes (C `struct pos_config` natural layout).
    ///
    ...
    pub fn from_bytes(bytes: &[u8]) -> Option<Self> {
        ...
        Some(Self {
            ...
            enabled: u32::from_le_bytes([bytes[16], bytes[17], bytes[18], bytes[19]]) != 0,
        })
    }
}
