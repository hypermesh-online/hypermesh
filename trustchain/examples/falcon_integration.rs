...
async fn main() -> Result<()> {
    ...
    // Sign with CA key
    let ca_signature = pqc
        .sign_with_falcon(test_data, &ca_keypair.private_key)
        .await?;
    info!(
        "✍️  CA Signature Created: {} bytes",
        ca_signature.signature_bytes.len()
    );

    // Sign with asset key
    let asset_signature = pqc
        .sign_with_falcon(test_data, &asset_keypair.private_key)
        .await?;
    info!(
        "✍️  Asset Signature Created: {} bytes",
        asset_signature.signature_bytes.len()
    );
    ...
    // Verify CA signature
    let ca_valid = pqc
        .verify_falcon_signature(test_data, &ca_signature, &ca_keypair.public_key)
        .await?;
    info!("🔍 CA Signature Valid: {}", ca_valid);

    // Verify asset signature
    let asset_valid = pqc
        .verify_falcon_signature(test_data, &asset_signature, &asset_keypair.public_key)
        .await?;
    info!("🔍 Asset Signature Valid: {}", asset_valid);

    // Cross-verification should fail
    let cross_valid = pqc
        .verify_falcon_signature(test_data, &ca_signature, &asset_keypair.public_key)
        .await?;
    ...
    info!(
        ...
        pq_info.quantum_security_level
    );
    info!(
        "🔐 Hybrid Signatures: {}",
        pq_info.hybrid_signatures_enabled
    );
    ...
    let encrypted_data = pqc
        .encrypt_with_kyber(secret_data, &kyber_keypair.public_key)
        .await?;
    info!(
    ...
    let decrypted_data = pqc
        .decrypt_with_kyber(&encrypted_data, &kyber_keypair.private_key)
        .await?;
