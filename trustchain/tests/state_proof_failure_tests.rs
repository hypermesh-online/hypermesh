...
use trustchain::proof_of_state::{
    hypermesh_client::{HyperMeshClientConfig, HyperMeshStateProofClient},
...
/// Test 5: Corrupted State Proof
#[tokio::test]
async fn test_corrupted_proof() -> Result<()> {
    init_failure_tracing();
    info!("=== Failure Test: Corrupted State Proof ===");

    let client_config = HyperMeshClientConfig::default();
    let hypermesh_client = HyperMeshStateProofClient::new(client_config).await?;

    // Create proof with corrupted/invalid data
    let corrupted_proof = StateProof::new_for_testing();
...
/// Test 7: Invalid State Proof Requirements
#[tokio::test]
async fn test_invalid_state_requirements() -> Result<()> {
    init_failure_tracing();
    info!("=== Failure Test: Invalid State Proof Requirements ===");
    ...
    match result {
        Err(e) => {
            ...
            info!("✅ Invalid state proof requirements handled");
        }
        Ok(_) => {
