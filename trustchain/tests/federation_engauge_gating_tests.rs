...
//! Phase F.1 — engauge → federation gating.
//!
...
//! according to the engauge-derived band.
...
#[tokio::test]
async fn test_engauge_signals_promote_to_full() {
    ...
    assert_eq!(
        ...
        "Full engauge band should preserve requested Full trust"
    );
}

#[tokio::test]
async fn test_engauge_signals_demote_to_conditional() {
    let fed = FederationManager::new("local-ca".into(), policy());
    fed.set_trust_signal_provider(Arc::new(StaticProvider {
    ...
    assert_eq!(
        ...
        "Conditional engauge band should cap peer at Conditional"
    );
}

#[tokio::test]
async fn test_no_signals_leaves_pos_level_unchanged() {
    // No provider attached → engauge gating is a no-op.  PoS validation
    ...
    assert_eq!(
        p.trust_level,
        FederationTrustLevel::Full,
        "without engauge gating, valid PoS retains requested Full trust"
    );
}
...
/// PoS failure trumps engauge promotion: a peer without proof is
/// Untrusted regardless of what engauge says.
#[tokio::test]
async fn test_pos_failure_trumps_engauge_full() {
    let fed = FederationManager::new("local-ca".into(), policy());
    fed.set_trust_signal_provider(Arc::new(StaticProvider {
        band: Some(PeerTrustBand::Full),
    }))
    ...
    assert_eq!(
        ...
        "PoS failure forces Untrusted regardless of engauge band"
    );
}
