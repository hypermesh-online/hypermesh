...
impl MetricsExporter for PrometheusExporter {
    async fn export(&self, snapshot: &MetricsSnapshot) -> anyhow::Result<String> {
        ...
        // Add header
        output.push(format!(
            "# HELP {}up TrustChain service status",
            self.prefix
        ));
        ...
        for (component, metrics) in &snapshot.components {
            ...
            // Total operations
            output.push(format!("# HELP {component_prefix}_total Total operations"));
            output.push(format!("# TYPE {component_prefix}_total counter"));
            output.push(format!(
                "{}_total {}",
                component_prefix, metrics.total_operations
            ));

            // Success rate
            output.push(format!(
                "# HELP {component_prefix}_success_rate Success rate"
            ));
            output.push(format!("# TYPE {component_prefix}_success_rate gauge"));
            output.push(format!(
                "{}_success_rate {}",
                component_prefix, metrics.success_rate
            ));

            // Average operation time
            output.push(format!(
                "# HELP {component_prefix}_avg_ms Average operation time in milliseconds"
            ));
            output.push(format!("# TYPE {component_prefix}_avg_ms gauge"));
            output.push(format!(
                "{}_avg_ms {}",
                component_prefix, metrics.avg_operation_time_ms
            ));

            // Additional metrics
            for (name, value) in &metrics.additional_metrics {
                let metric_name = format!("{component_prefix}_{name}");
                output.push(format!("# HELP {metric_name} {name}"));
                output.push(format!("# TYPE {metric_name} gauge"));
        ...
        for (name, value) in &snapshot.counters {
            ...
            output.push(format!("# HELP {counter_name} Counter: {name}"));
            output.push(format!("# TYPE {counter_name} counter"));
        ...
        for (name, stats) in &snapshot.timing_stats {
            ...
            output.push(format!("# HELP {timing_prefix}_min_ms Minimum time"));
            output.push(format!("# TYPE {timing_prefix}_min_ms gauge"));
            output.push(format!("{}_min_ms {}", timing_prefix, stats.min_ms));

            output.push(format!("# HELP {timing_prefix}_max_ms Maximum time"));
            output.push(format!("# TYPE {timing_prefix}_max_ms gauge"));
            output.push(format!("{}_max_ms {}", timing_prefix, stats.max_ms));

            output.push(format!("# HELP {timing_prefix}_avg_ms Average time"));
            output.push(format!("# TYPE {timing_prefix}_avg_ms gauge"));
            output.push(format!("{}_avg_ms {}", timing_prefix, stats.avg_ms));

            output.push(format!("# HELP {timing_prefix}_p95_ms 95th percentile"));
            output.push(format!("# TYPE {timing_prefix}_p95_ms gauge"));
            output.push(format!("{}_p95_ms {}", timing_prefix, stats.p95_ms));

            output.push(format!("# HELP {timing_prefix}_p99_ms 99th percentile"));
            output.push(format!("# TYPE {timing_prefix}_p99_ms gauge"));
...
impl MetricsExporter for PlainTextExporter {
    async fn export(&self, snapshot: &MetricsSnapshot) -> anyhow::Result<String> {
        ...
        output.push("TrustChain Metrics Report".to_string());
        output.push("========================".to_string());
        output.push(format!("Timestamp: {:?}", snapshot.timestamp));
        output.push(String::new());

        // Component metrics
        output.push("Component Metrics:".to_string());
        output.push("------------------".to_string());
        for (component, metrics) in &snapshot.components {
        ...
        if !snapshot.timing_stats.is_empty() {
            output.push("Timing Statistics:".to_string());
            output.push("------------------".to_string());
            for (name, stats) in &snapshot.timing_stats {
        ...
        if !snapshot.counters.is_empty() {
            output.push("Counters:".to_string());
            output.push("---------".to_string());
            for (name, value) in &snapshot.counters {
...
mod tests {
    ...
    fn create_test_snapshot() -> MetricsSnapshot {
        ...
        component
            .additional_metrics
            .insert("cert_count".to_string(), 1000.0);
    ...
    async fn test_json_exporter() {
        ...
        let result = exporter.export(&snapshot).await.expect("test: async operation");
