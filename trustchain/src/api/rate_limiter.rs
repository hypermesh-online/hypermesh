...
impl RateLimiter {
    ...
    pub async fn new(rate_limit_per_minute: u32) -> TrustChainResult<Self> {
        debug!(
            "Initializing rate limiter: {} requests/minute",
    ...
    pub async fn check_rate_limit(&self, client_id: &str) -> bool {
        ...
        debug!(
            "Rate limit check for {}: allowed={}, tokens={:.2}",
    ...
    pub async fn check_rate_limit_bulk(&self, client_id: &str, token_count: u32) -> bool {
        ...
        if !allowed {
            warn!(
                "Bulk rate limit exceeded for client: {} (requested {} tokens)",
                client_id, token_count
            );
        }

        debug!(
            "Bulk rate limit check for {}: allowed={}, tokens={:.2}, requested={}",
...
impl RateLimitError {
    pub fn new(client_id: String, current_limit: u32, remaining_tokens: f64) -> Self {
...
mod tests {
    ...
    async fn test_rate_limiter_creation() {
        let limiter = RateLimiter::new(60).await.expect("test: async operation");
    ...
    async fn test_rate_limit_exceed() {
        let limiter = RateLimiter::new(2).await.expect("test: async operation"); // Very low limit
    ...
    async fn test_rate_limit_refill() {
        let limiter = RateLimiter::new(60).await.expect("test: async operation"); // 1 token per second
    ...
    async fn test_multiple_clients() {
        let limiter = RateLimiter::new(5).await.expect("test: async operation");
    ...
    async fn test_bulk_rate_limit() {
        let limiter = RateLimiter::new(10).await.expect("test: async operation");
    ...
    async fn test_remaining_tokens() {
        ...
        assert!(
            (remaining - 8.0).abs() < 0.1,
            "Expected ~8.0 tokens remaining, got {remaining}"
        );
    }
    ...
    async fn test_clear_all() {
        ...
        // Clear all buckets
        limiter.clear_all().await.expect("test: async operation");
    ...
    async fn test_configurable_rate_limiter() {
        ...
        let limiter = ConfigurableRateLimiter::new(config).await.expect("test: async operation");
