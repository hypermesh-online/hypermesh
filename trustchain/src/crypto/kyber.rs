...
use super::{KyberKeyPair, KyberPrivateKey, KyberPublicKey, PQCError};
...
impl KyberCrypto {
    ...
    pub async fn encrypt(&self, data: &[u8], public_key: &KyberPublicKey) -> Result<Vec<u8>> {
        ...
        let public_key_native =
            kyber1024::PublicKey::from_bytes(&public_key.key_bytes).map_err(|e| {
                PQCError::KyberEncryptionError {
        ...
        combined_ciphertext
            .extend_from_slice(&(ciphertext_kem.as_bytes().len() as u32).to_be_bytes());
        combined_ciphertext.extend_from_slice(ciphertext_kem.as_bytes());
        combined_ciphertext.extend_from_slice(&encrypted_data);

        debug!(
            "✅ Kyber-1024 encryption completed: {} bytes total",
            combined_ciphertext.len()
        );
    ...
    pub async fn decrypt(
        ...
        debug!(
            "🔓 Decrypting {} bytes with Kyber-1024 KEM",
            ciphertext.len()
        );
        ...
        let secret_key_native =
            kyber1024::SecretKey::from_bytes(&private_key.key_bytes).map_err(|e| {
                PQCError::KyberDecryptionError {
                    message: format!("Failed to reconstruct Kyber-1024 secret key: {e}"),
                }
            })?;

        let ciphertext_native = kyber1024::Ciphertext::from_bytes(kem_ciphertext).map_err(|e| {
            PQCError::KyberDecryptionError {
        ...
        debug!(
            "✅ Kyber-1024 decryption completed: {} bytes",
            decrypted_data.len()
        );
        Ok(decrypted_data)
    }
    ...
    pub fn validate_keypair(&self, keypair: &KyberKeyPair) -> Result<bool> {
        ...
        if keypair.public_key.key_bytes.len() != kyber1024::public_key_bytes() {
            warn!(
                "❌ Invalid Kyber-1024 public key size: {}",
                keypair.public_key.key_bytes.len()
            );
            return Ok(false);
        }
        ...
        if keypair.private_key.key_bytes.len() != kyber1024::secret_key_bytes() {
            warn!(
                "❌ Invalid Kyber-1024 private key size: {}",
                keypair.private_key.key_bytes.len()
            );
            return Ok(false);
        }
...
mod tests {
    ...
    async fn test_kyber_crypto_initialization() {
        let kyber = KyberCrypto::new().expect("test: creation");
    ...
    async fn test_kyber_keypair_generation() {
        ...
        let keypair = kyber.generate_keypair().await.expect("test: async operation");
    ...
    async fn test_kyber_encrypt_decrypt_roundtrip() {
        ...
        // Encrypt the data
        let ciphertext = kyber.encrypt(test_data, &keypair.public_key).await.expect("test: async operation");
    ...
    async fn test_kyber_keypair_validation() {
        ...
        let is_valid = kyber.validate_keypair(&keypair).expect("test: validation");
        assert!(is_valid);
        ...
        let is_invalid = kyber.validate_keypair(&corrupted_keypair).expect("test: validation");
        assert!(!is_invalid);
    }
    ...
    async fn test_large_data_encryption() {
        ...
        let ciphertext = kyber
            .encrypt(&large_data, &keypair.public_key)
            .await
            .expect("test: expected success");
        let decrypted = kyber
            .decrypt(&ciphertext, &keypair.private_key)
    ...
    async fn test_multiple_keypairs() {
        ...
        let keypair1 = kyber.generate_keypair().await.expect("test: async operation");
        let keypair2 = kyber.generate_keypair().await.expect("test: async operation");
        ...
        let ciphertext1 = kyber
            .encrypt(test_data, &keypair1.public_key)
            .await
            .expect("test: expected success");
