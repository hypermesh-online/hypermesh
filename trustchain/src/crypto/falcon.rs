...
use super::{
    FalconKeyPair, FalconPrivateKey, FalconPublicKey, FalconSignature, KeyUsage, PQCError,
};
...
impl FalconCrypto {
    ...
    pub async fn generate_keypair(&self, usage: KeyUsage) -> Result<FalconKeyPair> {
        ...
        debug!(
            ...
            keypair.public_key.key_bytes.len()
        );
        debug!(
            "Private key size: {} bytes",
            keypair.private_key.key_bytes.len()
        );
        debug!(
            "Key fingerprint: {}",
    ...
    pub async fn sign(
        ...
        let secret_key_native =
            falcon1024::SecretKey::from_bytes(&private_key.key_bytes).map_err(|e| {
                PQCError::FalconSigningError {
    ...
    pub async fn verify(
        ...
        if signature.algorithm != self.algorithm_id {
            warn!(
                ...
                self.algorithm_id, signature.algorithm
            );
            return Ok(false);
        }
        ...
        let public_key_native =
            falcon1024::PublicKey::from_bytes(&public_key.key_bytes).map_err(|e| {
                PQCError::FalconVerificationError {
        ...
        let signature_native =
            falcon1024::DetachedSignature::from_bytes(&signature.signature_bytes).map_err(|e| {
                PQCError::FalconVerificationError {
    ...
    pub fn validate_keypair(&self, keypair: &FalconKeyPair) -> Result<bool> {
        ...
        if keypair.public_key.key_bytes.len() != falcon1024::public_key_bytes() {
            warn!(
                "❌ Invalid FALCON-1024 public key size: {}",
                keypair.public_key.key_bytes.len()
            );
            return Ok(false);
        }
        ...
        if keypair.private_key.key_bytes.len() != falcon1024::secret_key_bytes() {
            warn!(
                "❌ Invalid FALCON-1024 private key size: {}",
                keypair.private_key.key_bytes.len()
            );
            return Ok(false);
        }
        ...
        let test_signature = falcon1024::detached_sign(
            test_data,
            &falcon1024::SecretKey::from_bytes(&keypair.private_key.key_bytes).map_err(|e| {
                PQCError::InvalidKeyFormat {
        ...
        let verification_result = falcon1024::verify_detached_signature(
            &test_signature,
            test_data,
            &falcon1024::PublicKey::from_bytes(&keypair.public_key.key_bytes).map_err(|e| {
                PQCError::InvalidKeyFormat {
    ...
    pub fn extract_public_key_from_certificate(&self, _cert_der: &[u8]) -> Result<FalconPublicKey> {
        ...
        Err(anyhow!(
            "FALCON-1024 certificate integration under development"
        ))
    }
    ...
    pub fn create_csr_with_falcon(
        ...
        info!(
            "🚧 Creating FALCON-1024 certificate signing request for: {}",
            subject
        );
        ...
        Ok(format!(
            ...
            subject, san_entries, keypair.public_key
        )
        .into_bytes())
    }
...
mod tests {
    ...
    async fn test_falcon_crypto_initialization() {
        let falcon = FalconCrypto::new().expect("test: creation");
    ...
    async fn test_falcon_sign_verify_roundtrip() {
        ...
        // Sign the data
        let signature = falcon.sign(test_data, &keypair.private_key).await.expect("test: async operation");
        ...
        let is_invalid = falcon
            .verify(tampered_data, &signature, &keypair.public_key)
            .await
            .expect("test: expected success");
        assert!(!is_invalid);
    }
    ...
    async fn test_falcon_keypair_validation() {
        ...
        let is_valid = falcon.validate_keypair(&keypair).expect("test: validation");
        assert!(is_valid);
        ...
        let is_invalid = falcon.validate_keypair(&corrupted_keypair).expect("test: validation");
        assert!(!is_invalid);
    }
    ...
    async fn test_falcon_signature_sizes() {
        let falcon = FalconCrypto::new().expect("test: creation");
        let keypair = falcon
            .generate_keypair(KeyUsage::GeneralSigning)
            .await
            .expect("test: expected success");
        ...
        println!(
            "FALCON-1024 signature size: {} bytes",
            signature.signature_bytes.len()
        );
    }
    ...
    async fn test_multiple_keypairs() {
        ...
        assert_ne!(
            keypair1.private_key.key_bytes,
            keypair2.private_key.key_bytes
        );
        assert_ne!(
            keypair1.public_key.fingerprint,
            keypair2.public_key.fingerprint
        );

        // Cross-verification should fail
        let test_data = b"Cross-verification test";
        let signature1 = falcon.sign(test_data, &keypair1.private_key).await.expect("test: async operation");

        let is_cross_valid = falcon
            .verify(test_data, &signature1, &keypair2.public_key)
            .await
            .expect("test: expected success");
