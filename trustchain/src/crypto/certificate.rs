...
use super::{
    FalconKeyPair, FalconPublicKey, FalconSignature, KyberKeyPair, KyberPublicKey,
    PostQuantumCrypto,
};
...
impl PQCertificateManager {
    ...
    pub async fn new(ca_config: PQCAConfig) -> Result<Self> {
        info!(
            "🔐 Initializing post-quantum certificate manager for CA: {}",
            ca_config.ca_id
        );
    ...
    pub async fn generate_pq_csr(
        ...
        info!(
            "🔏 Generating post-quantum CSR for: {}",
            subject.common_name
        );
        ...
        params
            .distinguished_name
            .push(DnType::CommonName, subject.common_name.clone());
        ...
        if let Some(ref country) = subject.country {
            params
                .distinguished_name
                .push(DnType::CountryName, country.clone());
        }
        ...
        for san in &san_entries {
            if san.contains('@') {
                params
                    .subject_alt_names
                    .push(SanType::Rfc822Name(Ia5String::try_from(san.as_str())?));
            } else if let Ok(ip_addr) = san.parse::<std::net::IpAddr>() {
                params.subject_alt_names.push(SanType::IpAddress(ip_addr));
            } else {
                params
                    .subject_alt_names
                    .push(SanType::DnsName(Ia5String::try_from(san.as_str())?));
            }
        }
        ...
        let self_signature = self
            .pqc
            .sign_with_falcon(&csr_der, &keypair.private_key)
            .await?;
        ...
        info!(
            "✅ Post-quantum CSR generated for: {}",
            pq_csr.subject.common_name
        );
        Ok(pq_csr)
    }
    ...
    pub async fn issue_pq_certificate(&self, csr: PQCSR) -> Result<PQCertificate> {
        info!(
            "🔐 Issuing post-quantum certificate for: {}",
            csr.subject.common_name
        );

        // Verify CSR self-signature
        let csr_signature_valid = self
            .pqc
            .verify_falcon_signature(&csr.csr_der, &csr.self_signature, &csr.falcon_public_key)
            .await?;
        ...
        let validity_duration =
            std::time::Duration::from_secs(self.ca_config.cert_validity_days as u64 * 24 * 60 * 60);
        ...
        params
            .distinguished_name
            .push(DnType::CommonName, csr.subject.common_name.clone());
        ...
        let pq_extensions = PQCertificateExtensions {
            ...
            kyber_parameters: csr
                .kyber_public_key
                .as_ref()
                .map(|_| "Kyber-1024, NIST PQC standard".to_string()),
        ...
        let ca_signature = self
            .pqc
            .sign_with_falcon(&certificate_der, &self.ca_config.ca_keypair.private_key)
            .await?;
        ...
        info!(
            "✅ Post-quantum certificate issued: {}",
            pq_certificate.serial_number
        );
        Ok(pq_certificate)
    }
    ...
    pub async fn verify_pq_certificate(&self, certificate: &PQCertificate) -> Result<bool> {
        debug!(
            "🔍 Verifying post-quantum certificate: {}",
        ...
        if !self.verify_embedded_falcon_key(
            &certificate.certificate_der,
            &certificate.falcon_public_key,
        )? {
    ...
    pub async fn create_ca_certificate(&self) -> Result<PQCertificate> {
        info!(
            "🔐 Creating post-quantum CA certificate for: {}",
            self.ca_config.ca_id
        );
        ...
        info!(
            "✅ Post-quantum CA certificate created: {}",
    ...
    fn encode_falcon_public_key_extension(
        &self,
    ...
    fn encode_kyber_public_key_extension(
        &self,
    ...
    fn verify_embedded_falcon_key(
        &self,
        cert_der: &[u8],
        expected_key: &FalconPublicKey,
    ) -> Result<bool> {
...
mod tests {
    ...
    async fn test_pq_certificate_manager_initialization() {
        ...
        let cert_manager = PQCertificateManager::new(ca_config).await.expect("test: async operation");
    ...
    async fn test_pq_csr_generation() {
        let pqc = PostQuantumCrypto::new().expect("test: creation");
        let ca_keypair = pqc.generate_ca_keypair("test-ca").await.expect("test: async operation");
        let user_keypair = pqc.generate_asset_keypair().await.expect("test: async operation");
    ...
    async fn test_pq_certificate_issuance_and_verification() {
        ...
        // Issue certificate
        let certificate = cert_manager.issue_pq_certificate(csr).await.expect("test: async operation");

        assert_eq!(certificate.common_name, "test.example.com");
        assert!(!certificate.certificate_der.is_empty());

        // Verify certificate
        let is_valid = cert_manager
            .verify_pq_certificate(&certificate)
            .await
            .expect("test: expected success");
        assert!(is_valid);
    }
    ...
    async fn test_ca_certificate_creation() {
        ...
        let ca_cert = cert_manager.create_ca_certificate().await.expect("test: async operation");

        assert!(ca_cert.common_name.contains("TrustChain PQ-CA"));
        assert_eq!(ca_cert.pq_extensions.pq_cert_version, "CA-1.0");

        let is_valid = cert_manager.verify_pq_certificate(&ca_cert).await.expect("test: async operation");
        assert!(is_valid);
    }
}
