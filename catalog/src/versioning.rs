...
impl SemanticVersion {
    ...
    pub fn parse(version_str: &str) -> Result<Self> {
        ...
        let major = captures.name("major").expect("regex matched so major group exists").as_str().parse()?;
        let minor = captures.name("minor").expect("regex matched so minor group exists").as_str().parse()?;
        let patch = captures.name("patch").expect("regex matched so patch group exists").as_str().parse()?;
    ...
    pub fn satisfies(&self, constraint: &VersionConstraint) -> bool {
        match constraint {
            ...
            VersionConstraint::Tilde(version) => {
                ...
                self.major == version.major && self.minor == version.minor && self >= version
            }
...
impl VersionConstraint {
    ...
    pub fn parse(constraint_str: &str) -> Result<Self> {
        ...
        if let Some(rest) = constraint_str.strip_prefix("^") {
            let version = SemanticVersion::parse(rest)?;
            ...
            if parts.len() != 2 {
                return Err(anyhow::anyhow!(
                    "Invalid range constraint: {constraint_str}"
                ));
            }
...
impl VersionManager {
    ...
    pub fn get_latest_stable_version(&self, package_name: &str) -> Option<&SemanticVersion> {
        self.get_versions(package_name)
            .and_then(|versions| versions.iter().filter(|v| v.is_stable()).max())
    }
}
            ...
            if versions.len() > 1 {
                ...
                if unique_versions.len() > 1 {
                    conflicts.push(DependencyConflict {
                        ...
                        suggestion: format!(
                            "Choose a single version of {name} that satisfies all constraints"
                        ),
                    });
                }
            }
...
mod tests {
    ...
    fn test_semantic_version_parsing() {
        let version = SemanticVersion::parse("1.2.3").expect("test: expected success");
        assert_eq!(version.major, 1);
        assert_eq!(version.minor, 2);
        assert_eq!(version.patch, 3);
        assert!(version.pre_release.is_none());

        let pre_version = SemanticVersion::parse("1.2.3-alpha.1").expect("test: expected success");
        assert_eq!(pre_version.pre_release, Some("alpha.1".to_string()));

        let build_version = SemanticVersion::parse("1.2.3+build.1").expect("test: expected success");
    ...
    fn test_version_constraints() {
        ...
        let exact = VersionConstraint::parse("1.2.3").expect("test: expected success");
        assert!(version.satisfies(&exact));

        let compatible = VersionConstraint::parse("^1.2.0").expect("test: expected success");
        assert!(version.satisfies(&compatible));

        let tilde = VersionConstraint::parse("~1.2.0").expect("test: expected success");
        assert!(version.satisfies(&tilde));

        let greater = VersionConstraint::parse(">=1.0.0").expect("test: expected success");
    ...
    fn test_version_manager() {
        ...
        let latest = manager.get_latest_version("test-package").expect("test: expected success");
        assert_eq!(*latest, SemanticVersion::new(2, 0, 0));

        let constraint = VersionConstraint::parse("^1.0.0").expect("test: expected success");
        let best = manager
