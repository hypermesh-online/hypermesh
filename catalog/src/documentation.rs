...
*Generated by Catalog Documentation Generator*"#;
        ...
        self.templates
            .insert("readme".to_string(), readme_template.to_string());
...
<p>Version: {{asset.version}}</p>
        ...
        self.templates
            .insert("html".to_string(), html_template.to_string());
    ...
    async fn extract_lua_api(&self, content: &str) -> Result<serde_json::Value> {
        ...
        let function_regex =
            regex::Regex::new(r#"(?m)^(?:local\s+)?function\s+([^(]+)\s*\(([^)]*)\)"#)?;
        let table_regex = regex::Regex::new(r#"(?m)^(?:local\s+)?(\w+)\s*=\s*\{"#)?;

        // Extract functions
        for cap in function_regex.captures_iter(content) {
            let name = cap.get(1).expect("regex matched so capture group 1 exists").as_str();
            let params = cap.get(2).expect("regex matched so capture group 2 exists").as_str();

            // Look for documentation comment above function
            let function_start = cap.get(0).expect("regex matched so capture group 0 exists").start();
    ...
    async fn extract_usage_examples(&self, package: &AssetPackage) -> Result<serde_json::Value> {
        ...
        for pattern in &example_patterns {
            ...
            for cap in regex.captures_iter(&package.content.main_content) {
                let example_text = cap
                    .get(1)
                    .expect("regex matched so capture group 1 exists")
                    .as_str()
                    .lines()
    ...
    pub async fn save_to_directory<P: AsRef<Path>>(
        &self,
...
mod tests {
    ...
    async fn test_documentation_generation() {
        ...
        let generator = DocumentationGenerator::new(config).expect("test: creation");

        // Create a simple test package
        let package = create_test_package();

        let docs = generator.generate(&package).await.expect("test: async operation");
    ...
    async fn test_save_documentation() {
        ...
        generator
            .save_to_directory(&docs, temp_dir.path())
            .await
