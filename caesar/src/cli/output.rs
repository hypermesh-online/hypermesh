...
mod tests {
    ...
    fn test_cli_error_display() {
        let e = CliError::NotFound("packet-42".into());
        assert_eq!(format!("{e}"), "Not found: packet-42");

        let e2 = CliError::InvalidArgument("bad tier".into());
        assert_eq!(format!("{e2}"), "Invalid argument: bad tier");
    }
