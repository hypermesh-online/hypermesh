...
async fn test_matrix_position_registration_flow() -> Result<()> {
    ...
    println!(
        "Initialized node blockchain at position ({},{},{})",
        coordinate.x, coordinate.y, coordinate.z
    );
    ...
    println!(
        "✓ Position registered on blockchain in block: {}",
...
async fn test_neighbor_discovery_with_validation() -> Result<()> {
    ...
    let verification_results = validator
        .verify_neighbor_positions(&center, neighbor_coords.clone())
        .await?;
    ...
    println!(
        "✓ All {} neighbors validated successfully",
        verification_results.len()
    );

    // Verify unregistered position is invalid
    let unregistered = MatrixCoordinate::new(100, 100, 100)?;
    let unregistered_result = validator
        .verify_neighbor_positions(&center, vec![unregistered])
        .await?;

    assert_eq!(unregistered_result.len(), 1);
    assert!(
        !unregistered_result[0].1,
        "Unregistered position should be invalid"
    );
...
async fn test_pos_validation_requirements() -> Result<()> {
    ...
    let result = validator
        .register_position(coordinate, "strict_test_node".to_string(), proof.clone())
        .await;
    ...
    println!(
        "Registration with test proof in production mode: {:?}",
        result.is_ok()
    );

    // Now test with testing validator (relaxed requirements)
    let blockchain2 = Arc::new(NodeBlockchain::new(coordinate));
    let test_validator = MatrixPositionValidator::for_testing(blockchain2);

    let result2 = test_validator
        .register_position(
...
async fn test_network_level_validation() -> Result<()> {
    ...
    let topology_validation = network_validator
        .validate_topology_consistency(
            coordinate, 100.0, // radius
        )
        .await?;

    println!(
        "Topology validation: {} positions in radius, consistency score: {:.2}",
        topology_validation.positions_in_radius, topology_validation.consistency_score
    );
    ...
    println!(
        "Validation stats: {} total positions, {} cached, {:.2}% cache hit rate",
...
async fn test_central_vs_edge_position_requirements() -> Result<()> {
    ...
    println!(
        "Central position ({},{},{}) registered - requires higher stake/storage",
    ...
    println!(
        "Edge position ({},{},{}) registered - requires lower stake/storage",
