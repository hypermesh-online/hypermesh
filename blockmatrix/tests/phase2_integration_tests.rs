...
async fn test_content_storage_to_multinetwork_integration() {
    ...
    let handle = layer
        .process_asset(asset, PrivacyTier::Federated, networks.clone())
        .await
        .expect("Failed to process asset");
...
async fn test_e2e_asset_upload_private_tier() {
    ...
    assert_eq!(handle.privacy_tier, PrivacyTier::PrivateP2P);
...
async fn test_e2e_asset_upload_federated_tier() {
    ...
    assert_eq!(handle.privacy_tier, PrivacyTier::Federated);
...
async fn test_e2e_multi_network_asset_sharing() {
    ...
    let handle = layer
        .process_asset(asset.clone(), PrivacyTier::Federated, networks.clone())
        .await
        .expect("Failed to process asset");
