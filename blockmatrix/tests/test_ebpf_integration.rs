...
// Tests Linux eBPF kernel integration via the os_integration abstraction layer.
// Requires: Linux kernel 4.4+, CAP_BPF or root privileges
//
// Gated behind `future-tests` because:
//   - Tests require compiled eBPF object files (ebpf/*.o) that do not ship in the repo.
//   - OsAbstraction eBPF trait methods are implemented but not yet wired to real programs.
//   - XDP/kprobe/tracepoint attachment needs kernel privileges not available in CI.
...
mod ebpf_integration_tests {
    ...
    fn test_xdp_program_lifecycle() {
        ...
        Command::new("ping")
            ...
            .spawn()
            .expect("test: generate test traffic");
        ...
        let packet_count = metrics.values.get("packet_count").copied().unwrap_or(0);
        assert!(packet_count > 0, "No packets captured by XDP");
    ...
    fn test_interface_not_found_error() {
        ...
        // Attach and expect it may fail depending on system configuration.
        let attach_result = os.attach_ebpf_monitor(handle, EbpfAttachType::Xdp);
        // On systems without a suitable interface, this will error
        if let Err(e) = &attach_result {
            let error_msg = format!("{}", e);
            println!("XDP attach error (may be expected): {}", error_msg);
        }

        os.unload_ebpf_program(handle).expect("test: cleanup");
    }
    ...
    fn test_rapid_load_unload_cycles() {
        ...
        for i in 0..100 {
            ...
            os.unload_ebpf_program(handle)
                .expect(&format!("test: unload program iteration {}", i));
        }
...
// Export types for non-Linux test compilation
