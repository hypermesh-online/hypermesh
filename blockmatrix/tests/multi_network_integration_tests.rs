...
use blockmatrix::assets::multi_node::{
    EngagementEventType, IntegerMatrixPosition, MembershipStatus, MultiNetworkConfig,
    MultiNetworkCoordinator, NetworkDiscovery, NetworkId, PrivacyMode, TrustChainClient,
};
...
async fn test_join_multiple_networks_simultaneously() {
    ...
    let coordinator =
        MultiNetworkCoordinator::new(node, client.clone(), MultiNetworkConfig::default());

    // Discover networks via coordinator
    coordinator
        .discover_networks()
        .await
        .expect("test: discover 1");
    coordinator
        .discover_networks()
        .await
        .expect("test: discover 2");
    let networks = client
        .discover_networks()
        .await
        .expect("test: client discover");
    assert_eq!(networks.len(), 11); // 4 default + 7 extra

    // Join 10 networks (below the limit)
    for network in networks.iter().take(10) {
        let result = coordinator
            .join_network(network.network_id, network.privacy_tier)
            .await;
    ...
    assert!(!active.is_empty(), "Expected at least 1 active network");
...
async fn test_packet_isolation_zero_leakage() {
    ...
    coordinator
        .join_network(networks[0].network_id, PrivacyMode::PUBLIC)
        .await
        .expect("test: join 0");
    coordinator
        .join_network(networks[1].network_id, PrivacyMode::PRIVATE)
        .await
        .expect("test: join 1");
    ...
    assert_eq!(
        ...
        "CRITICAL: Packet leakage detected!"
    );
    assert!(report.strict_mode, "Strict isolation mode not enabled");

    println!(
        "Zero packet leakage confirmed - {} networks isolated",
...
async fn test_cross_network_asset_validation() {
    ...
    coordinator
        .add_asset_to_network(bank_network, car_title.clone(), position)
        .await
    ...
    let valid = coordinator
        .validate_asset_cross_network(car_title.clone(), bank_network, dealer_network, proof)
        .await
...
async fn test_car_purchase_scenario() {
    ...
    let car_position = IntegerMatrixPosition {
        x: 100,
        y: 50,
        z: 10,
    };
    coordinator
        .add_asset_to_network(bank_network, car_asset.clone(), car_position.clone())
        .await
        .expect("test: add asset bank");

    // Record engagement
    coordinator
        .record_engagement(bank_network, EngagementEventType::AssetUsed)
        .await;

    // Step 3: Bank validates on their blockchain
    println!("  3. Bank validating on their blockchain...");
    let bank_proof = create_test_proof();
    let bank_valid = coordinator
    ...
    coordinator
        .add_asset_to_network(dealer_network, car_asset.clone(), car_position.clone())
        .await
        .expect("test: add asset dealer");
    let dealer_valid = coordinator
        .validate_asset_cross_network(
            car_asset.clone(),
            bank_network,
            dealer_network,
        ...
        .await
        .expect("test: validate dealer");
    assert!(dealer_valid);
    coordinator
        .record_engagement(dealer_network, EngagementEventType::Transaction)
        .await;
    ...
    coordinator
        ...
        .await
        .expect("test: join insurance");

    // Step 7: Insurance validates
    println!("  7. Insurance validating...");
    coordinator
        .add_asset_to_network(insurance_network, car_asset.clone(), car_position.clone())
        .await
        .expect("test: add asset insurance");
    let insurance_valid = coordinator
        .validate_asset_cross_network(
            car_asset.clone(),
            dealer_network,
            insurance_network,
        ...
        .await
        .expect("test: validate insurance");
    assert!(insurance_valid);
    coordinator
        .record_engagement(insurance_network, EngagementEventType::Transaction)
        .await;
    ...
    coordinator
        ...
        .await
        .expect("test: join dmv");

    // Step 9: DMV validates for registration
    println!("  9. DMV validating for registration...");
    coordinator
        .add_asset_to_network(dmv_network, car_asset.clone(), car_position.clone())
        .await
        .expect("test: add asset dmv");
    let dmv_valid = coordinator
        .validate_asset_cross_network(
            car_asset.clone(),
            insurance_network,
            dmv_network,
        ...
        .await
        .expect("test: validate dmv");
    assert!(dmv_valid);
    coordinator
        .record_engagement(dmv_network, EngagementEventType::Transaction)
        .await;
    ...
    let isolation_report = coordinator
        .verify_isolation()
        .await
        .expect("test: verify isolation");
    assert_eq!(
        isolation_report.total_violations, 0,
        "CRITICAL: Isolation violations during car purchase!"
    );

    // Check engagement metrics
    let metrics = coordinator.get_engagement_metrics().await;
    println!(
        "  Engagement metrics collected for {} networks",
        metrics.len()
    );
...
async fn test_network_discovery() {
    ...
    println!(
        "Network discovery working - found {} networks",
        networks.len()
    );
}
...
async fn test_leave_network() {
    ...
    // Verify left
    let active = coordinator.active_networks().await;
    let still_member = active
        .iter()
        .any(|m| m.network_id == network_id && m.status == MembershipStatus::Active);
...
async fn test_max_networks_limit() {
    ...
    println!("Max networks limit enforced - joined {joined}/10");
}
