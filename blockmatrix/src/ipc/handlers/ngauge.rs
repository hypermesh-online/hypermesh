...
//! NGauge IPC handlers: capacity, traffic, throttle, routing.
//!
//! When the `intelligence` feature is enabled and an [`NGaugeBridge`] is wired
//! into [`DaemonState`], these handlers return real analytics from ngauge's
...
async fn handle_traffic(
    ...
    let (tracked_shards, transmit_summaries) = match &state.ngauge_bridge {
        Some(bridge) => {
...
async fn handle_throttle(
    ...
    Ok(serde_json::json!({
        ...
        "intelligence_enabled": cfg!(feature = "intelligence"),
        "bridge_attached": bridge_attached(state),
        "note": "throttle signal plumbing pending — ngauge MetricsIngestionPipeline→IPC",
    }))
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_ngauge_capacity_returns_json() {
        let state = super::super::tests::test_state().await;
        let mut handler = RequestHandler::new();
        register(&mut handler, &state);

        let req = RpcRequest::new("ngauge.capacity", serde_json::json!({}));
    ...
    #[tokio::test]
    async fn test_ngauge_traffic_returns_json() {
        let state = super::super::tests::test_state().await;
        let mut handler = RequestHandler::new();
        register(&mut handler, &state);

        let req = RpcRequest::new("ngauge.traffic", serde_json::json!({}));
        let resp = handler.dispatch(req).await;
        assert!(resp.error.is_none(), "unexpected error: {:?}", resp.error);
        let result = resp.result.expect("test: result present");
    ...
    #[tokio::test]
    async fn test_ngauge_throttle_returns_json() {
        let state = super::super::tests::test_state().await;
        let mut handler = RequestHandler::new();
        register(&mut handler, &state);

        let req = RpcRequest::new("ngauge.throttle", serde_json::json!({}));
        let resp = handler.dispatch(req).await;
        assert!(resp.error.is_none(), "unexpected error: {:?}", resp.error);
        let result = resp.result.expect("test: result present");
    ...
    #[tokio::test]
    async fn test_ngauge_routing_returns_json() {
        let state = super::super::tests::test_state().await;
        let mut handler = RequestHandler::new();
        register(&mut handler, &state);

        let req = RpcRequest::new("ngauge.routing", serde_json::json!({}));
        let resp = handler.dispatch(req).await;
        assert!(resp.error.is_none(), "unexpected error: {:?}", resp.error);
        let result = resp.result.expect("test: result present");
