...
use super::{ContainerSpec, NodeHealth, NodeState, ResourceRequirements};
...
impl DsrScheduler {
    ...
    pub async fn new(dsr_enabled: bool, max_candidates: usize) -> Result<Self> {
        info!("Initializing DSR container scheduler");
        info!(
            "  - DSR pattern matching: {}",
            if dsr_enabled { "enabled" } else { "disabled" }
        );
    ...
    pub async fn evaluate_node_candidates(
        ...
        debug!(
            "Evaluating {} candidate nodes for container {:?}",
            available_nodes.len(),
            spec.id
        );
        ...
        for node_id in nodes_to_evaluate {
            if let Some(node_state) = node_registry.get(&node_id) {
                ...
                let candidate = self
                    .evaluate_node_candidate(&node_id, node_state, spec, &placement_pattern)
                    .await?;
        ...
        self.update_scheduling_metrics(evaluation_latency, self.dsr_enabled)
            .await;

        debug!(
            "Evaluated {} candidates in {}ms, top score: {:.3}",
            candidates.len(),
            evaluation_latency,
    ...
    async fn generate_placement_pattern(
        ...
        pattern
            .push(spec.resources.storage_bytes as f64 / (1.0 * 1024.0 * 1024.0 * 1024.0 * 1024.0)); // Normalize to 1TB
        ...
        let healthy_nodes = node_registry
            .values()
    ...
    async fn evaluate_node_candidate(
        ...
        if let Some(cached_score) = self.check_node_scoring_cache(&cache_key).await {
            return Ok(NodeCandidate {
                ...
                expected_utilization: self
                    .calculate_expected_utilization(node_state, &spec.resources)
                    .await,
        ...
        let resource_fit = self
            .calculate_resource_fit(node_state, &spec.resources)
            .await;
        if resource_fit == 0.0 {
        ...
        let network_locality_score = self
            .calculate_network_locality_score(node_state, spec)
            .await;
        ...
        let final_score = (resource_fit * 0.25
            + affinity_score * 0.15
        ...
        // Cache the result
        self.cache_node_score(cache_key, final_score, rationale.clone())
            .await;
    ...
    async fn match_placement_pattern(
        &self,
        pattern: &[f64],
    ) -> Result<(f64, Option<PatternMatch>)> {
    ...
    /// Calculate resource fit score
    async fn calculate_resource_fit(
        &self,
        node_state: &NodeState,
        ...
        let cpu_utilization = (node_state.total_resources.cpu_cores - available.cpu_cores
            + requirements.cpu_cores)
            / node_state.total_resources.cpu_cores;
        let memory_utilization = (node_state.total_resources.memory_bytes - available.memory_bytes
            + requirements.memory_bytes) as f64
            / node_state.total_resources.memory_bytes as f64;

        // Base score for any fitting node (0.3) + bonus for balanced utilization near 70%
        let cpu_score = 0.3 + 0.7 * (1.0 - (cpu_utilization - 0.7).abs().min(1.0));
        let memory_score = 0.3 + 0.7 * (1.0 - (memory_utilization - 0.7).abs().min(1.0));

        (cpu_score + memory_score) / 2.0
    }

    /// Calculate affinity score
    async fn calculate_affinity_score(
        &self,
    ...
    async fn calculate_anti_affinity_score(
        &self,
        _node_state: &NodeState,
        _spec: &ContainerSpec,
    ) -> f64 {
    ...
    async fn calculate_network_locality_score(
        &self,
        node_state: &NodeState,
        _spec: &ContainerSpec,
    ) -> f64 {
    ...
    async fn calculate_expected_utilization(
        &self,
        node_state: &NodeState,
        requirements: &ResourceRequirements,
    ) -> ExpectedUtilization {
        ...
        let cpu_utilization =
            (total.cpu_cores - available.cpu_cores + requirements.cpu_cores) / total.cpu_cores;
        let memory_utilization = (total.memory_bytes - available.memory_bytes
            + requirements.memory_bytes) as f64
            / total.memory_bytes as f64;
        let storage_utilization = (total.storage_bytes - available.storage_bytes
            + requirements.storage_bytes) as f64
        ...
        let network_utilization = if total.network_bandwidth > 0 {
            (total.network_bandwidth - available.network_bandwidth
                + requirements.network_bandwidth.unwrap_or(0)) as f64
                / total.network_bandwidth as f64
        } else {
            0.0
        };
    ...
    async fn assess_placement_risks(
        &self,
        node_state: &NodeState,
        spec: &ContainerSpec,
    ) -> Vec<PlacementRisk> {
