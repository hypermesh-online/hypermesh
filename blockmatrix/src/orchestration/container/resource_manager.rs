...
impl IfrResourceManager {
    ...
    pub async fn new(ifr_enabled: bool) -> Result<Self> {
        info!("Initializing IFR resource manager");
        info!(
            "  - IFR resource lookup: {}",
            if ifr_enabled { "enabled" } else { "disabled" }
        );
    ...
    pub async fn find_suitable_nodes(
        ...
        debug!(
            "Finding nodes for resource requirements: CPU={}, Memory={}MB",
            requirements.cpu_cores,
            requirements.memory_bytes / (1024 * 1024)
        );
        ...
        if let Some(cached_result) = self.check_lookup_cache(&cache_key).await {
            self.update_lookup_metrics(lookup_start.elapsed().as_micros() as u64, false, true)
                .await;
        ...
        // Cache the result
        self.cache_lookup_result(cache_key, suitable_nodes.clone())
            .await;

        let lookup_latency = lookup_start.elapsed().as_micros() as u64;
        self.update_lookup_metrics(lookup_latency, self.ifr_enabled, false)
            .await;

        // Validate performance target (<52µs for IFR)
        if self.ifr_enabled && lookup_latency > 52 {
            warn!(
                "IFR resource lookup latency {}µs exceeds 52µs target",
                lookup_latency
            );
        } else {
            debug!(
                "Resource lookup completed in {}µs (target: {}µs)",
                lookup_latency,
                if self.ifr_enabled { 52 } else { 500 }
            );
        }

        debug!(
            "Found {} suitable nodes in {}µs",
            suitable_nodes.len(),
            lookup_latency
        );
        Ok(suitable_nodes)
    ...
    async fn traditional_resource_lookup(
        &self,
    ...
    pub async fn allocate_resources(
        ...
        info!(
            "Allocating resources on node {:?}: CPU={}, Memory={}MB",
            node_id,
            requirements.cpu_cores,
            requirements.memory_bytes / (1024 * 1024)
        );
        ...
        debug!(
            "Resource allocation {} created for node {:?}",
    ...
    pub async fn deallocate_resources(&self, node_id: &NodeId, allocation_id: &str) -> Result<()> {
        ...
        if let Some(node_allocations) = allocations.get_mut(node_id) {
            if node_allocations.remove(allocation_id).is_some() {
                info!(
                    "Deallocated resources {} from node {:?}",
                    allocation_id, node_id
                );
    ...
    pub async fn check_resource_fit(
        ...
        let available_network =
            node_resources.network_bandwidth - allocated_resources.network_bandwidth;
        ...
        let memory_fit = if requirements.memory_bytes <= available_memory {
            1.0 - (requirements.memory_bytes as f64
                / available_memory.max(requirements.memory_bytes) as f64)
        } else {
            0.0
        };

        let storage_fit = if requirements.storage_bytes <= available_storage {
            1.0 - (requirements.storage_bytes as f64
                / available_storage.max(requirements.storage_bytes) as f64)
        } else {
            0.0
        };
        ...
        let remaining_capacity = NodeResources {
            ...
            network_bandwidth: available_network
                .saturating_sub(requirements.network_bandwidth.unwrap_or(0)),
            custom_resources: HashMap::new(),
        };

        // Calculate utilization after allocation
        let cpu_utilization =
            (node_resources.cpu_cores - remaining_capacity.cpu_cores) / node_resources.cpu_cores;
        let memory_utilization = (node_resources.memory_bytes - remaining_capacity.memory_bytes)
            as f64
            / node_resources.memory_bytes as f64;
        let storage_utilization = (node_resources.storage_bytes - remaining_capacity.storage_bytes)
            as f64
            / node_resources.storage_bytes as f64;
        let gpu_utilization = if node_resources.gpu_units > 0 {
            (node_resources.gpu_units - remaining_capacity.gpu_units) as f64
                / node_resources.gpu_units as f64
        } else {
            0.0
        };
        let network_utilization = if node_resources.network_bandwidth > 0 {
            (node_resources.network_bandwidth - remaining_capacity.network_bandwidth) as f64
                / node_resources.network_bandwidth as f64
        } else {
            0.0
        };
    ...
    pub async fn add_resource_constraint(
        &self,
        node_id: &NodeId,
    ...
    pub async fn remove_resource_constraint(
        &self,
        node_id: &NodeId,
        constraint_id: &str,
    ) -> Result<()> {
        ...
        if let Some(node_constraints) = constraints.get_mut(node_id) {
            if let Some(pos) = node_constraints
                .iter()
                .position(|c| c.constraint_id == constraint_id)
            {
                node_constraints.remove(pos);
                info!(
                    "Removed resource constraint {} from node {:?}",
        ...
        warn!(
            "Resource constraint {} not found for node {:?}",
            constraint_id, node_id
        );
    ...
    async fn update_lookup_metrics(&self, latency_us: u64, ifr_enhanced: bool, cache_hit: bool) {
        ...
        if !cache_hit {
            ...
            metrics.avg_lookup_latency_us =
                (current_avg * (total_lookups - 1.0) + latency_us as f64) / total_lookups;
        ...
        metrics.cache_hit_rate = (metrics.cache_hit_rate * (total_requests - 1) as f64
            + cache_hits as f64)
