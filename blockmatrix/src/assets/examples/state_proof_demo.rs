...
//! Demonstration of StateProof integration with HyperMesh asset blocks
//!
//! This example shows how asset operations are validated through the unified
//! 4-proof state proof system (PoSp+PoSt+PoWk+PoTm) and stored in blockchain blocks.
...
use crate::proof_of_state::{
    ...
    NetworkPosition, AccessPermissions, AccessLevel, StateProofConfig,
};
use std::sync::Arc;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Starting StateProof integration demonstration");
    
    // 1. Create a test asset
    let asset_id = AssetRegistration::new(AssetType::Cpu);
    info!("Created asset ID: {}", asset_id.to_hex_string());
    
    // 2. Create all four state proofs following Proof of State patterns
    ...
    // 3. Create unified StateProof (all 4 proofs combined)
    ...
    info!("Created unified StateProof combining all 4 proof types");
    
    // 4. Validate the state proof
    match state_proof.validate().await {
        Ok(true) => info!("✅ StateProof validation successful"),
        Ok(false) => {
            warn!("❌ StateProof validation failed");
            return Ok(());
        }
        Err(e) => {
            warn!("❌ StateProof validation error: {:?}", e);
            return Ok(());
        }
    }
    ...
    let asset_record = HyperMeshAssetRecord::new(
        ...
        b"Demo asset creation with state proof validation".to_vec(),
        vec![state_proof],
        PrivacyMode::PUBLIC,
    );
    info!("Created HyperMeshAssetRecord with StateProof");
    
    // 6. Validate asset record state proof
    match asset_record.validate_state_proof().await {
        Ok(true) => info!("✅ Asset record state proof validation successful"),
        Ok(false) => {
            warn!("❌ Asset record state proof validation failed");
            return Ok(());
        }
        Err(e) => {
            warn!("❌ Asset record state proof validation error: {}", e);
            return Ok(());
        }
    }
    ...
    // Verify block data requires state proof
    if block_data.requires_state_proof() {
        info!("✅ Block data correctly requires state proof validation");
    } else {
        warn!("❌ Block data should require state proof validation");
    }
    ...
    // Note: This would normally use a real state proof system
    info!("StateProof integration demonstration completed successfully!");
    info!("");
    info!("Key Integration Points Demonstrated:");
    info!("  ✅ All 4 proofs (PoSp+PoSt+PoWk+PoTm) generated and validated");
    info!("  ✅ Asset records properly integrate with StateProof");
    ...
    info!("  ✅ Ready for integration with state proof engine");
