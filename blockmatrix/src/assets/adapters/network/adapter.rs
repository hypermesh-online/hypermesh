...
use crate::assets::core::{
    ...
    AssetType, BaseSystemType, StateProof, NetworkRequirements, NetworkScope, NetworkUsage,
...
impl NetworkAssetAdapter {
    ...
    async fn allocate_network_bandwidth(
                ...
                matches!(iface.status, InterfaceStatus::Up)
                    ...
                    && (network_req.protocols.is_empty()
                        || network_req
                            .protocols
                            .iter()
                            .all(|p| matches!(p.as_str(), "TCP" | "UDP" | "ICMP")))
            })
        ...
        for name in &suitable_interfaces {
            ...
            let iface = interfaces.get_mut(name).ok_or_else(|| {
                AssetError::ResourceUnavailable(format!("Interface {name} not found"))
            })?;
        ...
        if remaining > 0 {
            for name in &allocated_interfaces {
                ...
                iface.available_bandwidth_mbps +=
                    total_allocated_bandwidth / allocated_interfaces.len() as u64;
    ...
    async fn allocate_ipv6_addresses(&self, count: u32) -> Vec<String> {
        (0..count)
            .map(|i| format!("2001:db8:hypermesh::asset:{:x}", i + 1))
            .collect()
    }
    ...
    async fn configure_qos(
        ...
        let priority = if req.max_latency_us.unwrap_or(10000) < 1000 {
            200
        } else {
            128
        };
    ...
    async fn update_usage_stats(&self, operation: NetworkOperation, bandwidth_mbps: u64) {
        ...
        match operation {
            ...
            NetworkOperation::Deallocate => {
                ...
                stats.total_bandwidth_allocated = stats
                    .total_bandwidth_allocated
                    .saturating_sub(bandwidth_mbps);
            }
...
impl AssetAdapter for NetworkAssetAdapter {
    ...
    async fn validate_state_proof(&self, proof: &StateProof) -> AssetResult<bool> {
        ...
        if proof.stake_proof.stake_amount < 25 {
            return Ok(false);
        }
        if proof.work_proof.computational_power < 10 {
            return Ok(false);
        }
    ...
    async fn allocate_asset(
            ...
            .await?
        {
            return Err(AssetError::StateProofValidationFailed {
                reason: "Network allocation state proof validation failed".to_string(),
            });
        }

        let network_req = request
            .requested_resources
            .network_usage
            .as_ref()
        ...
        let (allocated_interfaces, allocated_bandwidth) = self
            .allocate_network_bandwidth(network_req, &asset_id)
            .await?;
        let proxy_address = Self::generate_proxy_address(&asset_id).await;
        let ipv6_addresses = self
            .allocate_ipv6_addresses(allocated_interfaces.len() as u32)
            .await;
        let qos_config = self.configure_qos(&asset_id, network_req).await;
        {
            self.state
                ...
                .write()
                .await
                .insert(asset_id.clone(), qos_config);
        }
        ...
        let allocation = NetworkAllocation {
            ...
            vlan_id: if matches!(request.privacy_level, PrivacyMode::PRIVATE) {
                let h = u128::from_le_bytes(asset_id.content_hash[..16].try_into().expect("16 bytes always fits into [u8; 16]"));
        ...
        self.update_usage_stats(NetworkOperation::Allocate, allocated_bandwidth)
            .await;
    ...
    async fn deallocate_asset(&self, asset_id: &AssetRegistration) -> AssetResult<()> {
...

        {
            ...
            let per_iface =
                allocation.allocated_bandwidth_mbps / allocation.allocated_interfaces.len() as u64;
    ...
    async fn get_asset_status(&self, asset_id: &AssetRegistration) -> AssetResult<AssetStatus> {
        ...
        Ok(AssetStatus {
            ...
            metadata: {
                ...
                m.insert(
                    "allocated_bandwidth_mbps".to_string(),
                    alloc.allocated_bandwidth_mbps.to_string(),
                );
                m.insert(
                    "interfaces".to_string(),
                    alloc.allocated_interfaces.len().to_string(),
                );
                m.insert("protocols".to_string(), alloc.enabled_protocols.join(","));
                m.insert("qos_priority".to_string(), alloc.qos_priority.to_string());
                m.insert(
                    "current_latency_us".to_string(),
                    alloc.current_latency_us.to_string(),
                );
                m.insert(
                    "packet_loss_percent".to_string(),
                    alloc.current_packet_loss_percent.to_string(),
                );
                m.insert(
                    "ipv6_addresses".to_string(),
                    alloc.ipv6_addresses.len().to_string(),
                );
                m.insert(
                    "vlan_id".to_string(),
                    alloc
    ...
    async fn configure_privacy_level(
        ...
        tracing::info!(
            "Updated privacy level for network asset {}: {:?}",
            asset_id,
            privacy
        );
        Ok(())
    ...
    async fn get_resource_usage(&self, asset_id: &AssetRegistration) -> AssetResult<ResourceUsage> {
        ...
        Ok(ResourceUsage {
            ...
            memory_usage: None,
            storage_usage: None,
            network_usage: Some(NetworkUsage {
                bytes_received: 0,
                bytes_transmitted: 0,
                packets_received: 0,
                packets_transmitted: 0,
                latency_us: Some(alloc.current_latency_us),
            }),
            measurement_timestamp: SystemTime::now(),
        })
    }
    ...
    async fn health_check(&self) -> AssetResult<AdapterHealth> {
        ...
        let failed = interfaces
            .values()
            .filter(|i| matches!(i.status, InterfaceStatus::Failed))
            .count();
        let down = interfaces
            .values()
            .filter(|i| matches!(i.status, InterfaceStatus::Down))
            .count();
        let healthy = failed == 0 && down < 2 && available > 0;

        let mut pm = HashMap::new();
        pm.insert(
            "total_bandwidth_gbps".to_string(),
            self.state.total_bandwidth as f64 / 1000.0,
        );
        pm.insert(
            "available_bandwidth_gbps".to_string(),
            available as f64 / 1000.0,
        );
        pm.insert(
            "bandwidth_utilization_percent".to_string(),
            ((self.state.total_bandwidth - available) as f64 / self.state.total_bandwidth as f64)
                * 100.0,
        );
        pm.insert(
        ...
        pm.insert(
            ...
            stats.average_latency_us as f64,
        );
        pm.insert(
            "average_packet_loss_percent".to_string(),
            stats.average_packet_loss_percent as f64,
        );
        ...
        Ok(AdapterHealth {
            ...
            message: if healthy {
                ...
                format!("Network adapter issues: {failed} failed, {down} down interfaces")
            },
