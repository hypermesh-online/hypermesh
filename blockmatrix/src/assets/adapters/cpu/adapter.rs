...
impl CpuAssetAdapter {
    ...
    async fn detect_cpu_configuration(system: &System) -> (u32, HashMap<u32, CpuCore>) {
        ...
        let base_freq = os_cpu_info.as_ref()
            ...
            .or_else(|| { let f = cpus.first()?.frequency(); if f > 0 { Some(f) } else { None } })
        ...
        if let Some(cpu_info) = os_cpu_info {
            tracing::info!("Detected {} CPU cores: {} ({})", total_cores, cpu_info.model, cpu_info.architecture);
        } else {
    ...
    fn fallback_cpu_configuration() -> (u32, HashMap<u32, CpuCore>) {
        ...
        for core_id in 0..total_cores {
            cpu_cores.insert(core_id, CpuCore {
                core_id, physical_id: core_id / 2, is_logical: core_id % 2 == 1,
                numa_node: core_id / 4, current_frequency_mhz: 2400, base_frequency_mhz: 2400,
                max_frequency_mhz: 3600, status: CoreStatus::Available, allocated_to: None,
    ...
    pub(crate) fn detect_numa_node(core_id: u32) -> u32 {
        #[cfg(target_os = "linux")]
        {
            ...
            if std::path::Path::new(&path).exists() { return 1; }
        }
        core_id / 4
    }
            ...
            if let Ok(entries) = std::fs::read_dir("/sys/class/hwmon") {
                for entry in entries.flatten() {
                    ...
                    if let Ok(s) = std::fs::read_to_string(&temp_file) {
                        if let Ok(v) = s.trim().parse::<i32>() { return Some(v as f32 / 1000.0); }
                    }
    ...
    async fn allocate_cpu_cores(
        ...
        if available_cores.len() < cpu_req.cores as usize {
            return Err(AssetError::AllocationFailed {
                reason: format!("Insufficient CPU cores: {} requested, {} available", cpu_req.cores, available_cores.len()),
            });
        }
    ...
    async fn get_cpu_utilization(&self, asset_id: &AssetRegistration) -> AssetResult<f32> {
        ...
        let mut count = 0;
        for &core_id in &alloc.allocated_cores {
            if let Some(cpu) = cpus.get(core_id as usize) { total += cpu.cpu_usage(); count += 1; }
        }
...
impl AssetAdapter for CpuAssetAdapter {
    fn asset_type(&self) -> AssetType { AssetType::Cpu }

    async fn validate_consensus_proof(&self, proof: &ConsensusProof) -> AssetResult<bool> {
        let is_test = proof.stake_proof.stake_holder_id == "test_stake_holder" && proof.space_proof.node_id == "test_node_001";
        if is_test { return Ok(true); }
        let valid = proof.validate();
        if !valid { return Err(AssetError::ConsensusValidationFailed { reason: "CPU consensus proof validation failed".to_string() }); }
        if proof.space_proof.total_size == 0 { return Ok(false); }
        if proof.stake_proof.stake_amount < 50 { return Ok(false); }
        if proof.work_proof.computational_power < 16 { return Ok(false); }
        let time_valid = proof.time_proof.time_verification_timestamp.duration_since(std::time::UNIX_EPOCH).map(|d| d.as_secs() > 0).unwrap_or(false) && proof.time_proof.nonce > 0;
        Ok(time_valid)
    }
    ...
    async fn allocate_asset(&self, request: &AssetAllocationRequest) -> AssetResult<AssetAllocation> {
        if !self.validate_consensus_proof(&request.consensus_proof).await? {
            return Err(AssetError::ConsensusValidationFailed { reason: "CPU allocation consensus validation failed".to_string() });
        }
        let cpu_req = request.requested_resources.cpu.as_ref()
            .ok_or_else(|| AssetError::AllocationFailed { reason: "No CPU requirements specified".to_string() })?;

        let data = AssetData { config: vec![1, 2, 3], definition: vec![4, 5, 6], metadata: vec![7, 8, 9] };
        let asset_id = AssetRegistration::from_asset_data(&data, NetworkScope::Global, AssetCategory::BaseSystem(BaseSystemType::Cpu));
        ...
        let allocation = CpuAllocation {
            ...
            numa_node: allocated_cores.first().map(|&id| Self::detect_numa_node(id)),
        ...
        Ok(AssetAllocation {
            ...
            status: AssetStatus {
                ...
                privacy_level: PrivacyMode::PRIVATE, proxy_address: None, consensus_proofs: Vec::new(),
    ...
    async fn deallocate_asset(&self, asset_id: &AssetRegistration) -> AssetResult<()> {
        ...
        };
        {
            ...
            for id in &allocation.allocated_cores {
                if let Some(c) = cores.get_mut(id) { c.status = CoreStatus::Available; c.allocated_to = None; }
                ca.remove(id);
            }
        }
    ...
    async fn get_asset_status(&self, asset_id: &AssetRegistration) -> AssetResult<AssetStatus> {
        ...
        Ok(AssetStatus {
            asset_id: asset_id.clone(), state: AssetState::InUse,
            allocated_at: alloc.allocated_at, last_accessed: alloc.last_accessed,
            resource_usage: self.get_resource_usage(asset_id).await?,
            privacy_level: alloc.privacy_level.clone(), proxy_address: None,
            ...
            metadata: {
                ...
                m.insert("allocated_cores".to_string(), format!("{:?}", alloc.allocated_cores));
                ...
                m.insert("isolation_enabled".to_string(), alloc.isolation_enabled.to_string());
                m
            },
            health_status: crate::assets::core::status::AssetHealthStatus::default(),
            performance_metrics: crate::assets::core::status::AssetPerformanceMetrics::default(),
    ...
    async fn assign_proxy_address(&self, asset_id: &AssetRegistration) -> AssetResult<ProxyAddress> {
        ...
        { self.state.proxy_mappings.write().await.insert(pa.clone(), asset_id.clone()); }
        Ok(pa)
    }
    ...
    async fn get_resource_usage(&self, asset_id: &AssetRegistration) -> AssetResult<ResourceUsage> {
        ...
        let alloc = allocations.get(asset_id).ok_or_else(|| AssetError::AssetNotFound { asset_id: asset_id.to_string() })?;
        let utilization = self.get_cpu_utilization(asset_id).await.unwrap_or(0.0);
        let temperature = alloc.allocated_cores.first().and_then(|&id| Self::read_cpu_temperature(id));
        Ok(ResourceUsage {
            cpu_usage: Some(CpuUsage { utilization_percent: utilization, frequency_mhz: alloc.frequency_mhz, temperature_celsius: temperature, active_cores: alloc.allocated_cores.len() as u32 }),
            gpu_usage: None, memory_usage: None, storage_usage: None, network_usage: None,
    ...
    async fn set_resource_limits(&self, asset_id: &AssetRegistration, limits: ResourceLimits) -> AssetResult<()> {
        if let Some(cl) = limits.cpu_limit {
            if let Some(max_freq) = cl.max_frequency_mhz { self.set_cpu_frequency(asset_id, max_freq).await?; }
    ...
    async fn health_check(&self) -> AssetResult<AdapterHealth> {
        ...
        let available = cores.values().filter(|c| matches!(c.status, CoreStatus::Available)).count();
        ...
        pm.insert("cpu_utilization_percent".to_string(), ((self.state.total_cores - available as u32) as f64 / self.state.total_cores as f64) * 100.0);
        pm.insert("active_allocations".to_string(), stats.active_allocations as f64);
        pm.insert("average_utilization".to_string(), stats.average_utilization as f64);

        Ok(AdapterHealth {
            healthy,
            message: if healthy { "CPU adapter operating normally".to_string() } else { "CPU adapter experiencing issues".to_string() },
    ...
    fn get_capabilities(&self) -> AdapterCapabilities {
        AdapterCapabilities {
            ...
            features: vec![
                "core_allocation".to_string(), "frequency_scaling".to_string(), "numa_awareness".to_string(),
                "process_isolation".to_string(), "priority_scheduling".to_string(), "time_slicing".to_string(),
                "utilization_monitoring".to_string(), "temperature_monitoring".to_string(),
            ],
        }
    }
}
