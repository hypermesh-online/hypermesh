...
pub use types::{
    MemoryAllocation, MemoryPool, MemoryProxyMapping, MemoryPermissions, MemoryUsageStats,
};
use types::{_MemoryAccessType, MemoryOperation};
...
impl AssetAdapter for MemoryAssetAdapter {
    ...
    async fn allocate_asset(&self, request: &AssetAllocationRequest) -> AssetResult<AssetAllocation> {
        ...
        let local_address = self.allocate_memory_from_pool(
            &pool_id, memory_req.size_bytes, memory_req.numa_node,
        ).await?;
    ...
    async fn set_resource_limits(&self, asset_id: &AssetRegistration, limits: ResourceLimits) -> AssetResult<()> {
        if let Some(memory_limit) = limits.memory_limit {
            tracing::info!(
                ...
                asset_id, memory_limit.max_bytes, memory_limit.max_swap_bytes
            );
        }
        Ok(())
    }
