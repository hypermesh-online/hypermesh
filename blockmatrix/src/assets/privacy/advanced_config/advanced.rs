    ...
    pub fn validate(&self) -> AssetResult<()> {
        if self.name.trim().is_empty() {
            return Err(AssetError::ValidationError {
                message: "Custom algorithm name cannot be empty".to_string(),
            });
        }
...
impl AlgorithmImplementation {
    pub fn validate(&self) -> AssetResult<()> {
        if self.language.trim().is_empty() {
            return Err(AssetError::ValidationError {
                message: "Implementation language cannot be empty".to_string(),
            });
        }

        if self.source.trim().is_empty() {
            return Err(AssetError::ValidationError {
                message: "Implementation source cannot be empty".to_string(),
            });
        }
...
impl FieldMapping {
    pub fn validate(&self) -> AssetResult<()> {
        if self.source_field.trim().is_empty() {
            return Err(AssetError::ValidationError {
                message: "Source field cannot be empty".to_string(),
            });
        }

        if self.target_field.trim().is_empty() {
            return Err(AssetError::ValidationError {
                message: "Target field cannot be empty".to_string(),
            });
        }
...
impl DataValidationRule {
    pub fn validate(&self) -> AssetResult<()> {
        if self.name.trim().is_empty() {
            return Err(AssetError::ValidationError {
                message: "Data validation rule name cannot be empty".to_string(),
            });
        }

        if self.expression.trim().is_empty() {
            return Err(AssetError::ValidationError {
                message: "Validation expression cannot be empty".to_string(),
            });
        }
...
impl ExperimentalFeature {
    pub fn validate(&self) -> AssetResult<()> {
        if self.name.trim().is_empty() {
            return Err(AssetError::ValidationError {
                message: "Experimental feature name cannot be empty".to_string(),
            });
        }
