...
pub struct ShardLocationIndex {
    locations: Arc<RwLock<HashMap<ContentHash, HashMap<String, ProviderEntry>>>>,
...
impl ShardLocationIndex {
    ...
    /// Register a node as a provider of the given shards, stamping (or
    /// refreshing) a TTL on each record. Re-registration from a fresh
    /// `TAG_SHARD_ANNOUNCE` extends the record's lifetime rather than
    ...
    /// Register with an explicit TTL (used by tests to force fast expiry).
    pub async fn register_provider_with_ttl(
        ...
        for shard_id in shard_ids {
            let providers = locs.entry(*shard_id).or_default();
            providers
