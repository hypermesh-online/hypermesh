...
pub struct DefaultContainerFilesystem {
    ...
    containers:
        std::sync::Arc<tokio::sync::RwLock<std::collections::HashMap<ContainerId, PathBuf>>>,
}

impl DefaultContainerFilesystem {
    pub fn new(storage_config: &StorageConfig) -> Result<Self> {
        std::fs::create_dir_all(&storage_config.containers).map_err(|e| {
            ContainerError::filesystem(format!("Failed to create containers directory: {e}"))
        })?;
    ...
    async fn create_container_filesystem(
        ...
        for path in [&container_path, &rootfs_path, &work_path, &upper_path] {
            std::fs::create_dir_all(path).map_err(|e| {
                ContainerError::filesystem(format!("Failed to create directory {path:?}: {e}"))
            })?;
        }
        ...
        for dir in ["bin", "etc", "tmp", "var", "usr"] {
            std::fs::create_dir_all(rootfs_path.join(dir)).map_err(|e| {
                ContainerError::filesystem(format!("Failed to create directory {dir}: {e}"))
            })?;
        }
        ...
        info!(
            "Created container filesystem for {} at {:?}",
            id, rootfs_path
        );
        Ok(rootfs_path)
    }
    ...
    async fn delete_container_filesystem(&self, id: &ContainerId) -> Result<()> {
        ...
        if container_path.exists() {
            std::fs::remove_dir_all(&container_path).map_err(|e| {
                ContainerError::filesystem(format!("Failed to delete container filesystem: {e}"))
            })?;
        }
            ...
            match modification {
                FileModification::Created {
                    ...
                    std::fs::write(&target_path, content).map_err(|e| {
                        ContainerError::filesystem(format!("Failed to write file: {e}"))
                    })?;
                }
                FileModification::Modified {
                    delta,
                    permissions: _,
                } => {
                    if target_path.exists() {
                        let mut existing = std::fs::read(&target_path).map_err(|e| {
                            ContainerError::filesystem(format!("Failed to read existing file: {e}"))
                        })?;
                        existing.extend_from_slice(delta);
                        std::fs::write(&target_path, existing).map_err(|e| {
                            ContainerError::filesystem(format!(
                ...
                FileModification::Deleted => {
                    if target_path.exists() {
                        std::fs::remove_file(&target_path).map_err(|e| {
                            ContainerError::filesystem(format!("Failed to delete file: {e}"))
                        })?;
                    }
                }
            }
