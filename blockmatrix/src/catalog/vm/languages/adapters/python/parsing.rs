...
//! Python consensus construct parsing
//!
//! Parses Python source code to identify consensus-related constructs such as
//! decorators, validation calls, asset operations, and P2P execution patterns.

use anyhow::Result;

use super::super::super::{
    ConsensusConstruct, ConsensusConstructType, SourceLocation, ProofRequirement,
    AssetDependency, AssetAccessPattern,
};

/// Parse Python-specific consensus constructs from source code
pub fn parse_python_consensus_constructs(code: &str) -> Result<Vec<ConsensusConstruct>> {
    ...
    for (line_num, line) in lines.iter().enumerate() {
        ...
        if line.trim().starts_with("@consensus_required") {
            let construct = parse_consensus_decorator(line, line_num as u32)?;
            constructs.push(construct);
        }

        // Parse consensus.validate() calls
        if line.contains("consensus.validate") || line.contains("ConsensusProof") {
            let construct = parse_consensus_validation(line, line_num as u32)?;
            constructs.push(construct);
        }
        ...
        if line.contains("CPUAsset") || line.contains("GPUAsset") ||
           line.contains("MemoryAsset") || line.contains("StorageAsset") {
            let construct = parse_asset_operation(line, line_num as u32)?;
            constructs.push(construct);
        }

        // Parse remote_execute calls
        if line.contains("remote_execute") {
            let construct = parse_p2p_execution(line, line_num as u32)?;
            constructs.push(construct);
        }
    }
...
/// Parse @consensus_required decorator
fn parse_consensus_decorator(line: &str, line_num: u32) -> Result<ConsensusConstruct> {
...
fn parse_consensus_validation(line: &str, line_num: u32) -> Result<ConsensusConstruct> {
    Ok(ConsensusConstruct {
        construct_type: ConsensusConstructType::ConsensusFunction,
        source_location: SourceLocation {
            line: line_num + 1,
