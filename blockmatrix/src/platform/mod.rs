...
pub use user_contribution::{
    ContributionId, ContributionSession, HardwareConfiguration, PlatformMetrics,
    SharingPreferences, UserContributionPlatform, UserId, UserProfile,
};
...
impl HyperMeshPlatform {
    ...
    pub async fn initialize() -> anyhow::Result<Self> {
        let asset_manager = std::sync::Arc::new(crate::assets::core::AssetManager::new());

        let platform_config = user_contribution::PlatformConfig::default();

        let contribution_platform =
            user_contribution::UserContributionPlatform::new(asset_manager, platform_config)
                .await?;
