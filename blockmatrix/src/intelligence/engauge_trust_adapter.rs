...
//! Engauge → TrustChain federation gating adapter (Phase F.1).
//!
...
//! maps SHA-256 fingerprints to engauge `PeerTrustSignals`, derives a
//! `TrustBand` via `PeerTrustSignals::trust_band`, and returns the
//! corresponding `PeerTrustBand`.
//!
//! Why a lookup table rather than a direct query of `EngaugeBridge`?
//! Engauge's `SwarmAnalytics` tracks shard-level popularity, not
...
use engauge::{PeerTrustSignals, TrustBand};
use trustchain::ca::trust_provider::{
    PeerCertFingerprint, PeerTrustBand, TrustSignalProvider,
};

/// Adapter exposing engauge-derived signals as a TrustChain
/// [`TrustSignalProvider`].
pub struct EngaugeTrustAdapter {
    /// Per-fingerprint signal cache.  Populated by the daemon (or by
    /// tests) as engauge accumulates measurements.
    signals: Arc<RwLock<HashMap<PeerCertFingerprint, PeerTrustSignals>>>,
}

impl Default for EngaugeTrustAdapter {
    fn default() -> Self {
        Self::new()
    }
}

impl EngaugeTrustAdapter {
...
#[async_trait]
impl TrustSignalProvider for EngaugeTrustAdapter {
...
mod tests {
    ...
    use engauge::metrics::ActivityScore;
    use engauge::organic_detection::TrafficClassification;
    ...
    async fn returns_full_for_high_signals() {
        let adapter = EngaugeTrustAdapter::new();
        let fp = [0xAB; 32];
