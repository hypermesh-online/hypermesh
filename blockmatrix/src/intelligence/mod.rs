...
use stoq::{Connection, Endpoint, NetworkIsolationManager, StoqTransport};

// Import Sprint 2.2: Privacy Tiers
use crate::assets::core::{
    AssetCategory, AssetData, AssetRegistration, BaseSystemType, NetworkScope, PrivacyMode,
};
use crate::assets::privacy::{CaesarRewardCalculator, PrivacyManager};

// Import Sprint 2.3: Multi-Network Participation
use crate::assets::multi_node::{CrossNetworkValidator, MultiNetworkCoordinator, NetworkId};
...
use crate::assets::storage::{ContentAddressedStorage, DeduplicationResult};
...
impl IntelligenceLayer {
    ...
    pub async fn new(
        ...
        let stoq_config = {
            let mut c = stoq::config::StoqConfig::default();
            // Use OS-assigned port to avoid conflicts when multiple tests run in parallel
            c.transport.port = 0;
            c.transport.bind_address = std::net::Ipv6Addr::LOCALHOST;
            c
        };
        ...
        let network_isolation = Arc::new(NetworkIsolationManager::new(isolation_config));
        ...
        let trustchain_client: Arc<
            dyn crate::assets::multi_node::network_membership::TrustChainClient,
        > = Arc::new(inline_trustchain_stub::StubTrustChainClient::new());
        let network_coordinator = Arc::new(MultiNetworkCoordinator::new(
            local_node_id,
            trustchain_client,
            network_config,
        ));
        ...
        let cross_validator = Arc::new(CrossNetworkValidator::new());
        ...
        let asset_pipeline = Arc::new(
            AssetPipeline::new(pipeline_config).context("Failed to initialize asset pipeline")?,
        );
        ...
        let performance_monitor = Arc::new(PerformanceMonitor::new(config.enable_monitoring));

        // Initialize integration validator
        let integration_validator = Arc::new(IntegrationValidator::new());
    ...
    pub async fn process_asset(
        ...
        let matrix_positions = self
            .calculate_optimal_positions(&processed, privacy_tier, networks.len())
            .await?;
        ...
        for network in &networks {
            ...
            if let Err(e) = self
                .network_coordinator
                .add_asset_to_network(*network, asset_id.clone(), matrix_pos)
                .await
            {
                warn!(
                    "Asset registration skipped for network (not a member): {:?}",
                    e
                );
            ...
            if self.config.enable_cross_network_validation {
                ...
                self.network_coordinator
                    .validate_asset_cross_network(asset_id, source_network, target_network, proof)
                    .await
        ...
        let shard_hashes: Vec<crate::assets::storage::Hash> = processed
            .shards
            .iter()
        ...
        self.update_processing_metrics(processing_time, &privacy_tier, &deduplicated_handles)
            .await;
        ...
        let avg_ref_count = deduplicated_handles
            .iter()
            .map(|h| h.reference_count)
            .sum::<usize>()
            / deduplicated_handles.len().max(1);
    ...
    pub async fn retrieve_asset(
        ...
        let start = Instant::now();
        info!(
            "Retrieving asset {} from position {:?}",
            handle.asset_id, requester_position
        );
        ...
        let instructions = self
            .content_storage
            .retrieve(handle.content_address.content_hash)
            .await
        ...
        info!(
            ...
            handle.asset_id, retrieval_time
        );
    ...
    pub async fn health_check(&self) -> Result<ValidationReport> {
        ...
        for (component, status) in &report.component_status {
            metrics.component_health.insert(component.clone(), *status);
        }
    ...
    async fn update_processing_metrics(
        ...
        if metrics.total_assets_processed == 1 {
            ...
            metrics.deduplication_rate = (metrics.deduplication_rate
                * (metrics.total_assets_processed - 1) as f64
                + dedup_rate)
    ...
    async fn update_retrieval_metrics(&self, retrieval_time: Duration) {
        ...
        if metrics.total_assets_retrieved == 1 {
            ...
            metrics.avg_retrieval_time_ms = (metrics.avg_retrieval_time_ms
                * (metrics.total_assets_retrieved - 1)
                + new_time_ms)
...
mod tests {
    ...
    async fn test_intelligence_layer_initialization() {
        ...
        let foundation_config =
            crate::integration::phase1_foundation::MatrixFoundationConfig::default();
