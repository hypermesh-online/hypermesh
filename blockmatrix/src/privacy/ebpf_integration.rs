...
use hypermesh_ebpf::policy_maps::{PolicyManager as EbpfPolicyManager, ValidationPolicy};
...
impl PrivacyEbpfBridge {
    ...
    pub fn update_ebpf_for_matrix(&self, matrix: &PrivacyFlexibilityMatrix, connection_id: u64) {
        ...
        let stricter_tier =
            if matrix.network_tier.caesar_multiplier() > matrix.asset_tier.caesar_multiplier() {
...
impl PrivacyEbpfMetrics {
    ...
    pub fn update_from_ebpf_event(&mut self, tier: u8, event_type: EbpfEventType) {
        match event_type {
            EbpfEventType::PacketFiltered => match tier {
    ...
    pub fn total_filtered(&self) -> u64 {
        self.anonymous_filtered
            + self.private_p2p_filtered
            + self.federated_filtered
            + self.public_filtered
    }
...
mod tests {
    ...
    fn test_tier_to_ebpf_policy_conversion() {
        let bridge = PrivacyEbpfBridge::new().expect("test: creation");
