...
/**
 * NGauge SSE streaming types and helpers (Phase M.6b).
 *
 * Mirrors the wire-format defined in `ngauge/src/streaming/protocol.rs`.
 * Frames are received over `GET /api/v1/blockmatrix/ngauge/stream` (and the
 * alias `/api/v1/ngauge/stream`) as `text/event-stream` `data:` payloads.
 */

import { useEventStream, type EventStreamState } from './sse';

// ---------------------------------------------------------------------------
// Wire types — keep in sync with ngauge protocol.rs
// ---------------------------------------------------------------------------
...
export const NGAUGE_SSE_PATH = '/api/v1/blockmatrix/ngauge/stream';

export interface UseNGaugeStreamOptions {
  /** Capability token forwarded as `X-HyperMesh-Capability`. Omit in alpha. */
  capability?: string;
  /** Pass `false` to disable the subscription. Default `true`. */
  enabled?: boolean;
  /** Override the SSE path. Default `/api/v1/blockmatrix/ngauge/stream`. */
  path?: string;
}

/**
 * Subscribe to the ngauge SSE metrics stream.
 *
 ...
 */
export function useNGaugeStream(
  opts: UseNGaugeStreamOptions = {},
): EventStreamState<MetricsFrame> {
  const { capability, enabled = true, path = NGAUGE_SSE_PATH } = opts;
