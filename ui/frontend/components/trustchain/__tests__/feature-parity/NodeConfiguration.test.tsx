...
import { NodeConfigurationSettings, type NodeSettings, type ConfigTestResult } from '../../../modules/trustchain/NodeConfigurationSettings';
...
const defaultTestResult: ConfigTestResult = {
  success: true,
  tests: {
    ipv6Connectivity: true,
    proxyAccess: true,
    bandwidthTest: { upload: 500, download: 800 },
    peerDiscovery: 12,
  },
  recommendations: [],
};

function renderNodeConfig(overrides: Partial<React.ComponentProps<typeof NodeConfigurationSettings>> = {}) {
  const defaultProps: React.ComponentProps<typeof NodeConfigurationSettings> = {
    nodeSettings: defaultNodeSettings,
    onSettingsChange: vi.fn(),
    onTest: vi.fn().mockResolvedValue(defaultTestResult),
    onSave: vi.fn().mockResolvedValue(undefined),
    onReset: vi.fn(),
  };

  return render(<NodeConfigurationSettings {...defaultProps} {...overrides} />);
}
...
describe('NodeConfigurationSettings - Feature Parity', () => {
  ...
  describe('State Management and Actions', () => {
    ...
    it('should handle save action correctly', async () => {
      ...
      renderNodeConfig({ onSave: mockOnSave });
    ...
    it('should handle test configuration action', async () => {
      const user = userEvent.setup();
      const mockOnTest = vi.fn().mockResolvedValue(defaultTestResult);
      renderNodeConfig({ onTest: mockOnTest });
    ...
    it('should handle reset to defaults', async () => {
      ...
      renderNodeConfig({ onReset: mockOnReset });
  ...
  describe('Loading States', () => {
    it('should handle loading state correctly', () => {
      renderNodeConfig({ isLoading: true });
