...
/**
 * State Proof Dashboard - Real-time Four-Proof state verification monitoring
 *
 * HyperMesh does NOT use consensus. It uses Proof of State -- bilateral binary
 * authentication. "There is no voting, no quorum, and no leader election.
 * Something is either authentic or it is not."
 *
 * Displays live state proof validation using the Proof of State Four-Proof system:
 ...
 *
 * Integrates with real HyperMesh state proof API endpoints for production data.
 */
...
export function StateProofDashboard() {
  ...
  // Get state proof history for the first asset (in production, this would be system-wide)
  const firstAssetId = assets?.[0]?.id;
  const { data: stateProofHistory } = useStateProofHistory(firstAssetId || '', 50);

  // Calculate real-time state proof metrics from API data
  const stateProofMetrics = React.useMemo((): StateProofMetrics => {
    ...
    const verificationRate = stateProofHistory.length > 0 ? (validValidations.length / stateProofHistory.length) * 100 : 0;

    // Calculate average block time from state proof history
    const blockTimes = stateProofHistory.slice(0, -1).map((entry, index) => {
      const currentTime = new Date(entry.timestamp).getTime();
      const nextTime = new Date(stateProofHistory[index + 1].timestamp).getTime();
    ...
    return {
      totalProofValidations: stateProofHistory.length,
      verificationRate,
      byzantineDetections: byzantineDetections?.length || 0,
      ...
      averageBlockTime,
      lastBlockHash: stateProofHistory[0]?.blockId || 'N/A'
    };
  }, [stateProofHistory, systemStatus, byzantineDetections]);

  // Calculate proof type statistics from state proof history
  const proofTypeStats = React.useMemo((): ProofTypeStats[] => {
    ...
    if (!stateProofHistory) return baseStats;

    // Calculate statistics for each proof type
    return baseStats.map(stat => {
      const proofEntries = stateProofHistory.filter(entry =>
  ...
  const handleTestStateProof = async () => {
    if (!firstAssetId) {
      alert('No assets available for state proof testing');
      return;
    }
    ...
    <div className="space-y-6">
      ...
      <div className="text-center py-6">
        <h1 className="text-3xl font-bold bg-gradient-to-r from-cyan-400 to-blue-600 bg-clip-text text-transparent mb-2">
          Four-Proof State Verification System
        </h1>
        <p className="text-gray-400 max-w-3xl mx-auto">
          Real-time monitoring of the Proof of State Four-Proof system. Every asset requires all four proofs:
          PoSp (WHERE), PoSt (WHO), PoWk (WHAT/HOW), and PoTm (WHEN) for complete validation.
        </p>
      </div>

      {/* State Proof Overview Metrics */}
      <div className="grid gap-4 md:grid-cols-4">
        <Card className="bg-black/40 border-cyan-500/30 backdrop-blur-lg">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-cyan-400">{stateProofMetrics.totalProofValidations}</div>
        ...
        <Card className="bg-black/40 border-green-500/30 backdrop-blur-lg">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-white">Verification Rate</CardTitle>
            <CheckCircle className="h-4 w-4 text-green-400" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-green-400">{stateProofMetrics.verificationRate.toFixed(1)}%</div>
        ...
        <Card className="bg-black/40 border-red-500/30 backdrop-blur-lg">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-400">{stateProofMetrics.byzantineDetections}</div>
        ...
        <Card className="bg-black/40 border-purple-500/30 backdrop-blur-lg">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-purple-400">{stateProofMetrics.networkParticipation.toFixed(1)}%</div>
          ...
          <Card className="bg-black/40 border-cyan-500/30 backdrop-blur-lg">
            <CardHeader>
              <CardTitle className="text-white flex items-center gap-2">
                ...
                State Proof System Status
              </CardTitle>
              <CardDescription className="text-gray-400">Real-time proof validation health and performance metrics</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <span className="text-white font-mono">{stateProofMetrics.averageBlockTime.toFixed(2)}s</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-400">Last Block Hash</span>
                    <span className="text-cyan-400 font-mono text-sm">{stateProofMetrics.lastBlockHash.slice(0, 12)}...</span>
                  </div>
                  <div className="flex justify-between">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between">
                    <span className="text-gray-400">Verification Participation</span>
                    <span className="text-green-400 font-mono">{stateProofMetrics.networkParticipation.toFixed(1)}%</span>
                  </div>
                  <div className="flex justify-between">
              ...
              <div className="pt-4 border-t border-cyan-500/20 flex gap-3">
                <Button
                  onClick={handleTestStateProof}
                  disabled={validateStateProof.isPending || !firstAssetId}
                  className="bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-black"
                >
                  {validateStateProof.isPending ? 'Testing...' : 'Test State Proof'}
                </Button>
                <Button
          ...
          <Card className="bg-black/40 border-cyan-500/30 backdrop-blur-lg">
            <CardHeader>
              <CardTitle className="text-white flex items-center gap-2">
                ...
                State Proof Validation History
              </CardTitle>
              <CardDescription className="text-gray-400">Recent state proof validation events and outcomes</CardDescription>
            </CardHeader>
            <CardContent>
              {stateProofHistory && stateProofHistory.length > 0 ? (
                <div className="space-y-3 max-h-96 overflow-y-auto">
                  {stateProofHistory.slice(0, 20).map((entry, index) => (
                ...
                <div className="text-center py-8">
                  ...
                  <h3 className="text-lg font-medium text-white mb-2">No State Proof History</h3>
                  <p className="text-gray-400">
                    {systemStatus ? 'No state proof validations have been performed yet.' : 'System offline - unable to load state proof history.'}
                  </p>
                </div>
              )}
            </CardContent>
