...
import type { JourneyStep, UserJourneyProps } from './user-journey';
...
export function UserJourney({
  ...
  if (compact) {
    return (
      <AccessibilityWrapper role="region" ariaLabel="User journey progress" className={cn('space-y-4', className)}>
        ...
        <Card className="bg-black/40 border-cyan-500/30 backdrop-blur-lg">
          ...
          <CardContent>
            {currentStep && (
              <NavigationElement
                id={`current-step-${currentStep.id}`} order={1}
                ...
                <div className="p-3 rounded-lg bg-cyan-500/10 border border-cyan-500/20 mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      {currentStep.progress !== undefined && <Progress value={currentStep.progress} className="mt-2 h-1" />}
                    </div>
            ...
            {nextSteps.length > 0 && (
              <div className="space-y-2">
                ...
                {nextSteps.map((step, index) => (
                  <NavigationElement
                    key={step.id} id={`next-step-${step.id}`} order={index + 2}
                  ...
                  >
                    <div className="flex items-center gap-3 p-2 rounded hover:bg-blue-500/10 transition-colors">
                      ...
                      <div className="flex-1">
                        ...
                        {step.estimatedTime && <p className="text-xs text-gray-500">{step.estimatedTime}</p>}
                      </div>
                    </div>
                  </NavigationElement>
                ))}
  ...
  return (
    <AccessibilityWrapper
      role="region" ariaLabel="Complete user journey through HyperMesh ecosystem"
    ...
    >
      ...
      <Card className="bg-black/40 border-cyan-500/30 backdrop-blur-lg">
        <CardHeader>
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-gray-400">{overallProgress.toFixed(0)}% through the ecosystem</p>
          </div>
        </CardHeader>
      </Card>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {steps.map((step, index) => {
          ...
          return (
            <NavigationElement
              key={step.id} id={`journey-step-${step.id}`} order={index + 10}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          {step.estimatedTime && <Badge variant="outline" className="text-xs">{step.estimatedTime}</Badge>}
                          {step.difficulty && (
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {step.requirements.map((req, i) => (
                          <Badge key={i} variant="outline" className="text-xs bg-yellow-500/20 text-yellow-400">{req}</Badge>
                        ))}
                      </div>
                    </div>
                  )}
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {step.rewards.map((reward, i) => (
                          <Badge key={i} variant="outline" className="text-xs bg-green-500/20 text-green-400">{reward}</Badge>
                        ))}
                      </div>
                    </div>
                  )}
      ...
      {showAchievements && <AchievementsSection achievements={achievements} />}
